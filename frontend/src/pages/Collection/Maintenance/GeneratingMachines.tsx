...
interface GeneratingMachinesProps {
  onLoadUsinas: () => Promise<Usina[]>;
  onLoadUnidades: (usinaId: number) => Promise<UnidadeGeradora[]>;
  onLoadMachines: (filters: { dataPdp?: string; usinaId?: number }) => Promise<MachineStatus[]>;
  onUpdateStatus: (machine: MachineStatus) => Promise<void>;
}

const GeneratingMachines: React.FC<GeneratingMachinesProps> = ({
  onLoadUsinas,
  onLoadUnidades,
  onLoadMachines,
  onUpdateStatus,
  ...
  const [formData, setFormData] = useState<Partial<MachineStatus>>({
    dataPdp: '',
    usinaId: 0,
    unidadeGeradoraId: 0,
    potenciaGerada: 0,
  ...
  const loadMachinesList = async () => {
    try {
      ...
      setError('Erro ao carregar máquinas gerando');
  ...
  const handleOpenDialog = (machine?: MachineStatus) => {
    if (machine) {
  ...
  const handleSave = async () => {
    try {
      ...
      const machineToSave: MachineStatus = {
        ...formData,
        id: editingId || 0,
  ...
  return (
    <div className={styles.container} data-testid="generating-machines-container">
      <div className={styles.header}>
        <h2>Coleta - Máquinas Gerando</h2>
      </div>
      ...
      <div className={styles.filters}>
        ...
        <button
          ...
          data-testid="btn-atualizar-status"
            ...
            {machines.length === 0 && !loading && (
              <tr>
                <td colSpan={7} className={styles.noData}>
                  Nenhuma máquina gerando encontrada
                </td>
              </tr>
            )}
            {loading && (
        ...
        <div className={styles.modal} data-testid="dialog-form">
          <div className={styles.modalContent}>
            <div className={styles.modalHeader}>
              <h3>{editingId ? 'Editar Status' : 'Novo Status'}</h3>
              <button
...
export default GeneratingMachines;
