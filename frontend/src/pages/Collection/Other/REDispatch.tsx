...
import styles from './REDispatch.module.css';
import { REDispatchData, REDispatchTableRow, REDispatchFormData } from '../../../types/reDispatch';

/**
 * Componente para Coleta de Despacho de Reserva de Energia (RE)
 * Registra motivos relativos ao despacho de reserva energética de usinas térmicas
 * RE = Reserva de energia disponível para atender variações de carga e contingências
 */
const REDispatch: React.FC = () => {
  const [formData, setFormData] = useState<REDispatchFormData>({
    dataPDP: '',
    empresa: '',
    usina: ''
  });

  const [tableData, setTableData] = useState<REDispatchTableRow[]>([]);
  ...
  const handleFormChange = (field: keyof REDispatchFormData, value: string) => {
  ...
  const loadREDispatchData = (usina: string) => {
    const mockData: string[] = [];
    
    if (usina === 'Todas as Usinas') {
      for (let i = 0; i < 48; i++) {
        mockData.push('20\t35\t50');
      }
    } else {
      for (let i = 0; i < 48; i++) {
        mockData.push('20');
  ...
  const handleSave = () => {
    ...
    console.log('Salvando Despacho RE:', {
      formData,
      tableData
    });

    alert('Despacho de Reserva de Energia salvo com sucesso!');
  };
    ...
    <div className={styles.container}>
      <div className={styles.header}>
        ...
        <div className={styles.pageTitle}>
          <img src="/images/tit_ColMotivorel.gif" alt="Despacho de Reserva de Energia" />
        </div>
      </div>
          ...
          {showTextarea && (
            <div className={styles.textareaOverlay}>
              <textarea
                ...
                placeholder="Digite valores de Reserva de Energia (MW) por intervalo"
              />
            </div>
          )}
        </div>
