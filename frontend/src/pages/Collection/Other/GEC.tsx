...
import styles from './GEC.module.css';
import { CompanyDTO, PlantDTO, TableRowData, GECData } from '../../../types/gec';
...
const buildMockData = (plants: PlantDTO[], isAll: boolean): GECData[] => {
  ...
  return targets.flatMap((plant, plantIndex) =>
    Array.from({ length: 48 }, (_, i) => ({
      ...
      intGec: i + 1,
      valGecTran: Number(((i + 1) * (plantIndex + 1)).toFixed(2)),
    }))
  );
};
...
const GEC: React.FC = () => {
  ...
  const buildTableData = (data: GECData[]) => {
    const intervals = new Map<number, TableRowData>();

    data.forEach((item) => {
      if (!intervals.has(item.intGec)) {
        intervals.set(item.intGec, {
          intervalo: item.intGec,
          valores: {},
          total: 0,
          media: 0,
        });
      ...
      const row = intervals.get(item.intGec)!;
      row.valores[item.codUsina] = item.valGecTran;
      row.total += item.valGecTran;
    });
  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      setError('Erro ao carregar dados de GEC');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleValueChange = (intervalo: number, codUsina: string, rawValue: string) => {
    ...
    let nextError = '';
    if (trimmed === '') {
      nextError = 'Valor de GEC é obrigatório';
    } else if (hasNegativeSign || parsed < 0) {
      nextError = 'Valores de GEC não podem ser negativos';
    } else if (Number.isNaN(parsed)) {
      nextError = 'Valor inválido de GEC';
    }
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      console.log('Salvando GEC', tableData);

      setSuccessMessage('Dados de GEC salvos com sucesso');
      setDataModified(false);
      setTimeout(() => setSuccessMessage(''), 2500);
    } catch (err) {
      setError('Erro ao salvar dados de GEC');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className={styles.container} data-testid="gec-container">
      <div className={styles.card}>
        <h1 className={styles.title} data-testid="gec-title">
          Coleta de Geração de Energia Contratada (GEC)
        </h1>
        <p className={styles.subtitle} data-testid="gec-subtitle">
          Registro dos valores de GEC por usina e intervalo de 30 minutos (48 intervalos)
        </p>
        ...
        {showTable && tableData.length > 0 && (
          <div className={styles.tableWrapper} data-testid="table-wrapper">
            ...
            <div className={styles.tableScroll}>
              <table className={styles.table} data-testid="gec-table">
                <thead>
                  <tr>
