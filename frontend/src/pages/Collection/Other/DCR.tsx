...
import { DCRData, DCRTableRow, DCRFormData } from '../../../types/dcr';

/**
 * Componente para Coleta de DCR (Despacho Ciclo Reduzido)
 * Registra dados de despacho de usinas termelétricas com operação reduzida
 * Ciclo Reduzido = operação abaixo da capacidade nominal por restrições operacionais
 */
const DCR: React.FC = () => {
  ...
  const [tableData, setTableData] = useState<DCRTableRow[]>([]);
  ...
  const loadDCRData = (usina: string) => {
    const mockData: string[] = [];
    
    if (usina === 'Todas as Usinas') {
      for (let i = 0; i < 48; i++) {
        mockData.push('30\t45\t60');
      }
    } else {
      for (let i = 0; i < 48; i++) {
        mockData.push('30');
  ...
  const handleSave = () => {
    ...
    console.log('Salvando dados DCR:', {
      formData,
      tableData
    });

    alert('Dados DCR salvos com sucesso!');
  };
    ...
    <div className={styles.container}>
      <div className={styles.header}>
        ...
        <div className={styles.pageTitle}>
          <h1 className={styles.titulo}>Despacho Ciclo Reduzido</h1>
        </div>
      </div>
          ...
          {showTextarea && (
            <div className={styles.textareaOverlay}>
              <textarea
                ...
                placeholder="Digite os valores DCR (MW) - operação reduzida por restrições"
              />
            </div>
          )}
        </div>
