...
import styles from './OutOfMeritCRE.module.css';
import { OutOfMeritCREData, OutOfMeritCRETableRow, OutOfMeritCREFormData } from '../../../types/outOfMeritCRE';

/**
 * Componente para Coleta de CRE Fora de Mérito
 * Registra Contratação de Reserva Energética operando fora da ordem de mérito econômico
 * CRE Fora de Mérito = despacho por restrições operacionais, não por custo
 */
const OutOfMeritCRE: React.FC = () => {
  const [formData, setFormData] = useState<OutOfMeritCREFormData>({
    dataPDP: '',
    empresa: '',
    usina: ''
  });

  const [tableData, setTableData] = useState<OutOfMeritCRETableRow[]>([]);
  ...
  const initializeTable = () => {
    const rows: OutOfMeritCRETableRow[] = [];
    for (let i = 1; i <= 48; i++) {
      rows.push({
        intervalo: i,
  ...
  const handleFormChange = (field: keyof OutOfMeritCREFormData, value: string) => {
  ...
  const loadOutOfMeritCREData = (usina: string) => {
    const mockData: string[] = [];
    
    if (usina === 'Todas as Usinas') {
      for (let i = 0; i < 48; i++) {
        mockData.push('10\t20\t30');
      }
    } else {
      for (let i = 0; i < 48; i++) {
        mockData.push('10');
  ...
  const handleSave = () => {
    ...
    console.log('Salvando CRE Fora de Mérito:', {
      formData,
      tableData
    });

    alert('CRE Fora de Mérito salvo com sucesso!');
  };
    ...
    <div className={styles.container}>
      <div className={styles.header}>
        ...
        <div className={styles.pageTitle}>
          <img src="/images/tit_CreForaMerito.gif" alt="CRE Fora de Mérito" />
        </div>
      </div>
          ...
          {showTextarea && (
            <div className={styles.textareaOverlay}>
              <textarea
                ...
                placeholder="Digite valores CRE Fora de Mérito (MW) - despacho fora da ordem econômica"
              />
            </div>
          )}
        </div>
