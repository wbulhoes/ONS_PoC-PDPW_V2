...
import { CompensationData, CompensationTableRow, CompensationFormData } from '../../../types/compensation';

/**
 * Componente para Coleta de Compensação de Lastro Físico (CLF)
 * Registra compensações de lastro físico de geração entre usinas
 * CLF = Compensação de Lastro Físico para ajustes de contratos
 */
const Compensation: React.FC = () => {
  ...
  const [tableData, setTableData] = useState<CompensationTableRow[]>([]);
  ...
  const initializeTable = () => {
    const rows: CompensationTableRow[] = [];
    for (let i = 1; i <= 48; i++) {
      rows.push({
        intervalo: i,
  ...
  const handleFormChange = (field: keyof CompensationFormData, value: string) => {
  ...
  const loadCompensationData = (usina: string) => {
    const mockData: string[] = [];
    
    if (usina === 'Todas as Usinas') {
      for (let i = 0; i < 48; i++) {
        mockData.push('15\t25\t40');
      }
    } else {
      for (let i = 0; i < 48; i++) {
        mockData.push('15');
  ...
  const handleSave = () => {
    ...
    console.log('Salvando Compensação de Lastro Físico:', {
      formData,
      tableData
    });

    alert('Compensação de Lastro Físico salva com sucesso!');
  };
    ...
    <div className={styles.container}>
      <div className={styles.header}>
        ...
        <div className={styles.pageTitle}>
          <img src="/images/tit_ColCLF.gif" alt="Compensação de Lastro Físico" />
        </div>
      </div>
          ...
          {showTextarea && (
            <div className={styles.textareaOverlay}>
              <textarea
                ...
                placeholder="Digite valores de Compensação (MW) - ajustes contratuais de lastro físico"
              />
            </div>
          )}
        </div>
