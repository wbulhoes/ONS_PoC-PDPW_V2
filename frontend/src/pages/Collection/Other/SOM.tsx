...
import { SOMData, SOMTableRow, SOMFormData } from '../../../types/som';

/**
 * Componente para Coleta de SOM (Sistema de Operação em Malha)
 * Registra dados de operação em malha de usinas por intervalo de 30 minutos
 * SOM representa a operação coordenada de múltiplas usinas em um sistema interligado
 */
const SOM: React.FC = () => {
  const [formData, setFormData] = useState<SOMFormData>({
    dataPDP: '',
    empresa: '',
    usina: ''
  });

  const [tableData, setTableData] = useState<SOMTableRow[]>([]);
  ...
  useEffect(() => {
    initializeTable();
    // Simula carregamento de datas disponíveis
    setDataOptions(['01/01/2024', '02/01/2024', '03/01/2024']);
  }, []);

  const handleFormChange = (field: keyof SOMFormData, value: string) => {
  ...
  const handleUsinaChange = (value: string) => {
    ...
    if (value && value !== 'Selecione uma Usina') {
      // Simula carregamento de dados da usina
      loadSOMData(value);
  ...
  const loadSOMData = (usina: string) => {
    // Simula carregamento de dados do backend
    const mockData: string[] = [];
    
    if (usina === 'Todas as Usinas') {
      // Múltiplas colunas (usinas)
      for (let i = 0; i < 48; i++) {
        mockData.push('100\t150\t200'); // 3 usinas exemplo
      }
    } else {
      // Uma única usina
      for (let i = 0; i < 48; i++) {
  ...
  const handleSave = () => {
    ...
    console.log('Salvando dados SOM:', {
      formData,
      tableData
    });

    alert('Dados SOM salvos com sucesso!');
  };
    ...
    <div className={styles.container}>
      <div className={styles.header}>
        ...
        <div className={styles.pageTitle}>
          <img src="/images/tit_som.gif" alt="SOM - Sistema de Operação em Malha" />
        </div>
      </div>
          ...
          {showTextarea && (
            <div className={styles.textareaOverlay}>
              <textarea
                ...
                placeholder="Digite os valores SOM separados por Tab (múltiplas usinas) ou Enter (próximo intervalo)"
              />
            </div>
          )}
        </div>
