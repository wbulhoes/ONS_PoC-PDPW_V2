...
import styles from './DadosTermicos.module.css';

const DadosTermicos: React.FC = () => {
  ...
  const carregarDados = async () => {
    ...
    try {
      ...
      setDados(resultado.filter(d => d.status === 'Termico'));
    } catch (err) {
      setError('Erro ao carregar dados térmicos');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setError(null);
    try {
      await dadosEnergeticosService.criar({ ...form, status: 'Termico' });
      setForm({ dataReferencia: '', codigoUsina: '', producaoMWh: 0, capacidadeDisponivel: 0, status: 'Termico', observacoes: '' });
      await carregarDados();
    } catch (err) {
      setError('Erro ao salvar dado térmico');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className={styles.container}>
      ...
      <h2>Consulta de Dados Térmicos</h2>
