...
import GeneratingMachines from '../../src/pages/Collection/Maintenance/GeneratingMachines';
import type { MachineStatus } from '../../src/types/machineStatus';

describe('GeneratingMachines Component', () => {
  const mockUsinas = [
  ...
  describe('Renderização', () => {
    it('deve renderizar o componente sem erros', () => {
      ...
      expect(screen.getByText('Coleta - Máquinas Gerando')).toBeInTheDocument();
    });
    ...
    it('deve renderizar botão de atualizar status', () => {
      render(
        <GeneratingMachines
          onLoadUsinas={mockOnLoadUsinas}
          onLoadUnidades={mockOnLoadUnidades}
      ...
      expect(screen.getByTestId('btn-atualizar-status')).toBeInTheDocument();
    });
  });
  ...
  describe('Carregamento de dados', () => {
    ...
    it('deve exibir mensagem quando não há máquinas gerando', async () => {
      ...
      await waitFor(() => {
        expect(screen.getByText('Nenhuma máquina gerando encontrada')).toBeInTheDocument();
      });
    });
  });

  describe('Diálogo de formulário', () => {
    it('deve abrir diálogo ao clicar em atualizar status', async () => {
      render(
        <GeneratingMachines
          onLoadUsinas={mockOnLoadUsinas}
          onLoadUnidades={mockOnLoadUnidades}
      ...
      const btnNovo = screen.getByTestId('btn-atualizar-status');
    ...
    it('deve fechar diálogo ao clicar em cancelar', async () => {
      ...
      fireEvent.click(screen.getByTestId('btn-atualizar-status'));
