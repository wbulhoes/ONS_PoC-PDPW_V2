Partial Class frmColDespInflex
...
#End Region
    ...
    Private Sub cboEmpresa_SelectedIndexChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cboEmpresa.SelectedIndexChanged
        ...
        Try
            ...
            If Not ValidaLimiteEntradaDados(cboEmpresa.SelectedItem.Value, cboData.SelectedItem.Value, lRetorno, "MIF") Then
    ...
    Private Sub cboUsina_SelectedIndexChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cboUsina.SelectedIndexChanged
        ...
        Try
            If cboUsina.SelectedItem.Text = "Todas as Usinas" Then
                Cmd.CommandText = "SELECT d.valmiftran, d.intmif, d.codusina, u.ordem " &
                                  "FROM motivoinfl d, usina u " &
            ...
            Else
                Cmd.CommandText = "SELECT d.valmiftran, d.intmif " &
                                  "FROM motivoinfl d, usina u " &
                                  "WHERE u.codempre = '" & cboEmpresa.SelectedItem.Value & "' " &
                                  "AND u.tipusina = 'T' " &
                                  "AND u.codusina = d.codusina " &
            ...
            Do While rsUsina.Read
                If blnPassou Then
                    If rsUsina("intmif") <> intInstante Then
                        intInstante = rsUsina("intmif")
                        objTextArea.Value = objTextArea.Value & Chr(13)
                    Else
                        objTextArea.Value = objTextArea.Value & Chr(9)
                    End If
                End If
                If Not IsDBNull(rsUsina.Item("valmiftran")) Then
                    objTextArea.Value = objTextArea.Value & rsUsina.Item("valmiftran")
                Else
    ...
    Private Sub PreencheTable()
        ...
        Dim strCodUsina As String
        Try
            Cmd.CommandText = "SELECT d.codusina, d.intmif, d.valmiftran, u.ordem " &
                              "FROM motivoinfl d, usina u " &
                              "WHERE u.codempre = '" & Session("strCodEmpre") & "' " &
                              "AND u.tipusina = 'T' " &
                              "AND u.codusina = d.codusina " &
                              "AND d.datpdp = '" & Format(Session("datEscolhida"), "yyyyMMdd") & "' " &
                              "AND u.flg_recebepdpage = 'S' " &
                              "ORDER BY u.ordem, d.codusina, d.intmif"
            Conn.Open()
            Dim rsUsina As System.Data.SqlClient.SqlDataReader = Cmd.ExecuteReader
            tblmotivoinfl.Rows.Clear()
            ...
            tblmotivoinfl.Width = objTamanho.Pixel(70)
            tblmotivoinfl.Controls.Add(objRow)
            ...
            Do While rsUsina.Read
                If strCodUsina <> rsUsina.Item("codusina") Then
                    ...
                    objCel.Font.Bold = True
                    tblmotivoinfl.Rows(0).Width = objTamanho.Pixel(tblmotivoinfl.Rows(0).Width.Value + 64)
                    tblmotivoinfl.Width = objTamanho.Pixel(tblmotivoinfl.Width.Value + 64)
                    ...
                    tblmotivoinfl.Rows(0).Controls.Add(objCel)
                ...
                objCel.Width = objTamanho.Pixel(64)
                If Not IsDBNull(rsUsina.Item("valmiftran")) Then
                    objCel.Text = rsUsina.Item("valmiftran")
                    dblMedia = dblMedia + rsUsina.Item("valmiftran")
                Else
                    objCel.Text = 0
                End If
                tblmotivoinfl.Rows(intLin).Width = objTamanho.Pixel(tblmotivoinfl.Rows(intLin).Width.Value + 63)
                tblmotivoinfl.Rows(intLin).Controls.Add(objCel)
                intLin = intLin + 1
            Loop
            rsUsina.Close()
            rsUsina = Nothing
    ...
    Private Sub btnSalvar_Click(ByVal sender As System.Object, ByVal e As System.Web.UI.ImageClickEventArgs) Handles btnSalvar.Click
        ...
        'objPermissao = New OnsClasses.OnsInterfaceUnica.OnsPermissao("UPD", "PDPColDespInflex", UsuarID)
        ...
        If EstaAutorizado Then
            ...
            Try
                ...
                'Grava evento registrando o recebimento de Motivo Despacho Inflexibilidade
                GravaEventoPDP("48", Format(Session("datEscolhida"), "yyyyMMdd"), cboEmpresa.SelectedItem.Text, datAtual, "PDPColDespInflex", UsuarID)
