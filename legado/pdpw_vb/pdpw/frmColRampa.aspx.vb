...
#End Region
    ...
    Private Sub cboEmpresa_SelectedIndexChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cboEmpresa.SelectedIndexChanged
        ...
        Try
            ...
            If Not ValidaLimiteEntradaDados(cboEmpresa.SelectedItem.Value, cboData.SelectedItem.Value, lRetorno, "RMP") Then
    ...
    Private Sub cboUsina_SelectedIndexChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cboUsina.SelectedIndexChanged
        ...
        Try
            If cboUsina.SelectedItem.Text = "Todas as Usinas" Then
                Cmd.CommandText = "SELECT d.valrmptran, d.intrmp, d.codusina, u.ordem " &
                                  "FROM tb_rmp d, usina u " &
                                  "WHERE u.codempre = '" & cboEmpresa.SelectedItem.Value & "' " &
                                  "AND u.codusina = d.codusina " &
                                  "AND d.datpdp = '" & Format(Session("datEscolhida"), "yyyyMMdd") & "' " &
                                  "AND u.flg_recebepdpage = 'S' " &
                                  "ORDER BY d.intrmp, u.ordem, d.codusina"
            Else
                Cmd.CommandText = "SELECT d.valrmptran, d.intrmp " &
                                  "FROM tb_rmp d, usina u " &
                                  "WHERE u.codempre = '" & cboEmpresa.SelectedItem.Value & "' " &
                                  "AND u.codusina = d.codusina " &
            ...
            Do While rsUsina.Read
                If blnPassou Then
                    If rsUsina("intrmp") <> intInstante Then
                        intInstante = rsUsina("intrmp")
                        objTextArea.Value = objTextArea.Value & Chr(13)
                    Else
                        objTextArea.Value = objTextArea.Value & Chr(9)
                    End If
                End If
                If Not IsDBNull(rsUsina.Item("valrmptran")) Then
                    objTextArea.Value = objTextArea.Value & rsUsina.Item("valrmptran")
                Else
    ...
    Private Sub PreencheTable()
        ...
        Dim strCodUsina As String
        Try
            Cmd.CommandText = "SELECT d.codusina, d.intrmp, d.valrmptran, u.ordem " &
                              "FROM tb_rmp d, usina u " &
                              "WHERE u.codempre = '" & Session("strCodEmpre") & "' " &
                              "AND u.codusina = d.codusina " &
                              "AND d.datpdp = '" & Format(Session("datEscolhida"), "yyyyMMdd") & "' " &
                              "AND u.flg_recebepdpage = 'S' " &
                              "ORDER BY u.ordem, d.codusina, d.intrmp "
            Conn.Open("pdp")
            Dim rsUsina As OnsClasses.OnsData.OnsDataReader = Cmd.ExecuteReader
            tblRMP.Rows.Clear()
            ...
            objRow.Controls.Add(objCel)
            tblRMP.Width = objTamanho.Pixel(132)
            tblRMP.Controls.Add(objRow)
            ...
            Do While rsUsina.Read
                If strCodUsina <> rsUsina.Item("codusina") Then
                    ...
                    objCel.Font.Bold = True
                    tblRMP.Rows(0).Width = objTamanho.Pixel(tblRMP.Rows(0).Width.Value + 64)
                    tblRMP.Width = objTamanho.Pixel(tblRMP.Width.Value + 64)
                    ...
                    tblRMP.Rows(0).Controls.Add(objCel)
                ...
                objCel.Width = objTamanho.Pixel(64)
                If Not IsDBNull(rsUsina.Item("valrmptran")) Then
                    objCel.Text = rsUsina.Item("valrmptran")
                    dblMedia = dblMedia + rsUsina("valrmptran")
                Else
                    objCel.Text = 0
                End If
                tblRMP.Rows(intLin).Width = objTamanho.Pixel(tblRMP.Rows(intLin).Width.Value + 63)
                tblRMP.Rows(intLin).Controls.Add(objCel)
                intLin = intLin + 1
                If intLin = 49 Then
                    objCel = New TableCell
                    objCel.Wrap = False
                    ...
                    tblRMP.Rows(intLin).Width = objTamanho.Pixel(tblRMP.Rows(intLin).Width.Value + 64)
                    tblRMP.Rows(intLin).Controls.Add(objCel)
                    tblRMP.Rows(intLin).Cells(1).Text = Trim(Str(Val(tblRMP.Rows(intLin).Cells(1).Text) + dblMedia)) '(dblMedia / 2)))

                    objCel = New TableCell
                    objCel.Wrap = False
                    objCel.Width = objTamanho.Pixel(64)
                    objCel.Text = Trim(Str(Int(dblMedia / 48)))
                    tblRMP.Rows(intLin + 1).Width = objTamanho.Pixel(tblRMP.Rows(intLin + 1).Width.Value + 64)
                    tblRMP.Rows(intLin + 1).Controls.Add(objCel)
                    tblRMP.Rows(intLin + 1).Cells(1).Text = Trim(Str(Val(tblRMP.Rows(intLin + 1).Cells(1).Text) + Int(dblMedia / 48)))
            ...
            For intI = 1 To 50
                dblMedia = 0
                For intJ = 2 To tblRMP.Rows(0).Cells.Count - 1
                    dblMedia = dblMedia + Val(tblRMP.Rows(intI).Cells(intJ).Text)
                Next
                tblRMP.Rows(intI).Cells(1).Text = dblMedia '/ 2
            Next
            rsUsina.Close()
            rsUsina = Nothing
            'Cmd.Connection.Close()
    ...
    Private Sub btnSalvar_Click(ByVal sender As System.Object, ByVal e As System.Web.UI.ImageClickEventArgs) Handles btnSalvar.Click
        ...
        'objPermissao = New OnsClasses.OnsInterfaceUnica.OnsPermissao("UPD", "PDPColRmp", UsuarID)
        ...
        If EstaAutorizado Then
            ...
            Try
                ...
                If strValor = "" Then
                    ...
                    If cboUsina.SelectedItem.Text <> "Todas as Usinas" Then
                        For intI = 1 To 48
                            ...
                            Cmd.CommandText = "UPDATE tb_rmp " & _
                                              "SET valrmptran = 0 " & _
                ...
                'Grava evento registrando o recebimento de Garantia Energética (RMP)
                GravaEventoPDP("52", Format(Session("datEscolhida"), "yyyyMMdd"), cboEmpresa.SelectedItem.Text, datAtual, "PDPColRmp", UsuarID)
