...
Partial Class frmColResFaltaComb
...
#End Region
    ...
    Private Sub cboEmpresa_SelectedIndexChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cboEmpresa.SelectedIndexChanged
        ...
        Try
            ...
            If Not ValidaLimiteEntradaDados(cboEmpresa.SelectedItem.Value, cboData.SelectedItem.Value, lRetorno, "RFC") Then
    ...
    Private Sub cboUsina_SelectedIndexChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cboUsina.SelectedIndexChanged
        ...
        Try
            If cboUsina.SelectedItem.Text = "Todas as Usinas" Then
                Cmd.CommandText = "SELECT d.valrfctran, d.intrfc, d.codusina, u.ordem " &
                                  "FROM rest_falta_comb d, usina u " &
            ...
            Else
                Cmd.CommandText = "SELECT d.valrfctran, d.intrfc " &
                                  "FROM rest_falta_comb d, usina u " &
                                  "WHERE u.codempre = '" & cboEmpresa.SelectedItem.Value & "' " &
                                  "AND u.tipusina = 'T' " &
                                  "AND u.codusina = d.codusina " &
            ...
            Do While rsUsina.Read
                If blnPassou Then
                    If rsUsina("intrfc") <> intInstante Then
                        intInstante = rsUsina("intrfc")
                        objTextArea.Value = objTextArea.Value & Chr(13)
                    Else
                        objTextArea.Value = objTextArea.Value & Chr(9)
                    End If
                End If
                If Not IsDBNull(rsUsina.Item("valrfctran")) Then
                    objTextArea.Value = objTextArea.Value & rsUsina.Item("valrfctran")
                Else
    ...
    Private Sub PreencheTable()
        ...
        Dim strCodUsina As String
        Try
            Cmd.CommandText = "SELECT d.codusina, d.intrfc, d.valrfctran, u.ordem " &
                              "FROM rest_falta_comb d, usina u " &
                              "WHERE u.codempre = '" & Session("strCodEmpre") & "' " &
                              "AND u.tipusina = 'T' " &
                              "AND u.codusina = d.codusina " &
                              "AND d.datpdp = '" & Format(Session("datEscolhida"), "yyyyMMdd") & "' " &
                              "AND u.flg_recebepdpage = 'S' " &
                              "ORDER BY u.ordem, d.codusina, d.intrfc"
            Conn.ConnectionString = ConfigurationManager.AppSettings.Get("pdpSQL").ToString()
            Conn.Open()
            Dim rsUsina As SqlDataReader = Cmd.ExecuteReader
            tblResFaltaComb.Rows.Clear()
            ...
            objRow.Controls.Add(objCel)
            tblResFaltaComb.Width = objTamanho.Pixel(132)
            tblResFaltaComb.Controls.Add(objRow)
            ...
            Do While rsUsina.Read
                If strCodUsina <> rsUsina.Item("codusina") Then
                    ...
                    objCel.Font.Bold = True
                    tblResFaltaComb.Rows(0).Width = objTamanho.Pixel(tblResFaltaComb.Rows(0).Width.Value + 64)
                    tblResFaltaComb.Width = objTamanho.Pixel(tblResFaltaComb.Width.Value + 64)
                    ...
                    tblResFaltaComb.Rows(0).Controls.Add(objCel)
                ...
                objCel.Width = objTamanho.Pixel(64)
                If Not IsDBNull(rsUsina.Item("valrfctran")) Then
                    objCel.Text = rsUsina.Item("valrfctran")
                    dblMedia = dblMedia + rsUsina.Item("valrfctran")
                Else
                    objCel.Text = 0
                End If
                tblResFaltaComb.Rows(intLin).Width = objTamanho.Pixel(tblResFaltaComb.Rows(intLin).Width.Value + 63)
                tblResFaltaComb.Rows(intLin).Controls.Add(objCel)
                intLin = intLin + 1
                If intLin = 49 Then
                    objCel = New TableCell
                    objCel.Wrap = False
                    ...
                    tblResFaltaComb.Rows(intLin).Width = objTamanho.Pixel(tblResFaltaComb.Rows(intLin).Width.Value + 64)
                    tblResFaltaComb.Rows(intLin).Controls.Add(objCel)
                    tblResFaltaComb.Rows(intLin).Cells(1).Text = Trim(Str(Val(tblResFaltaComb.Rows(intLin).Cells(1).Text) + dblMedia))

                    objCel = New TableCell
                    objCel.Wrap = False
                    objCel.Width = objTamanho.Pixel(64)
                    objCel.Text = Trim(Str(Int(dblMedia / 48)))
                    tblResFaltaComb.Rows(intLin + 1).Width = objTamanho.Pixel(tblResFaltaComb.Rows(intLin + 1).Width.Value + 64)
                    tblResFaltaComb.Rows(intLin + 1).Controls.Add(objCel)
                    tblResFaltaComb.Rows(intLin + 1).Cells(1).Text = Trim(Str(Val(tblResFaltaComb.Rows(intLin + 1).Cells(1).Text) + Int(dblMedia / 48)))
            ...
            For intI = 1 To 50
                dblMedia = 0
                For intJ = 2 To tblResFaltaComb.Rows(0).Cells.Count - 1
                    dblMedia = dblMedia + Val(tblResFaltaComb.Rows(intI).Cells(intJ).Text)
                Next
                tblResFaltaComb.Rows(intI).Cells(1).Text = dblMedia '/ 2
            Next
    ...
    Private Sub btnSalvar_Click(ByVal sender As System.Object, ByVal e As System.Web.UI.ImageClickEventArgs) Handles btnSalvar.Click
        ...
        'objPermissao = New OnsClasses.OnsInterfaceUnica.OnsPermissao("UPD", "PDPColResFalCom", UsuarID)
        ...
        If EstaAutorizado Then
            ...
            Try
                    ...
                    If cboUsina.SelectedItem.Text <> "Todas as Usinas" Then
                        For intI = 1 To 48
                            ...
                            Cmd.CommandText = "UPDATE rest_falta_comb " &
                                              ...
                                              "AND codusina = '" & Trim(cboUsina.SelectedItem.Value) & "' " &
                                              "AND intrfc = " & intI
                            Cmd.ExecuteNonQuery()
                        Next
                    Else
                        For intI = 1 To 48
                            For intJ = 1 To cboUsina.Items.Count - 2
                                ...
                                Cmd.CommandText = "UPDATE rest_falta_comb " &
                                                  "SET valrfctran = 0 " &
                                                  "WHERE datPdp = '" & Format(Session("datEscolhida"), "yyyyMMdd") & "' " &
                                                  "AND codusina = '" & Trim(cboUsina.Items(intJ).Value) & "' " &
                                                  "AND intrfc = " & intI
                                Cmd.ExecuteNonQuery()
                            Next
                        Next
                        ...
                        For intI = 1 To 48
                            ...
                            Do While InStr(intColAnterior, strLinha, Chr(9), CompareMethod.Binary) <> 0
                                ...
                                Cmd.CommandText = "UPDATE rest_falta_comb " &
                                                  "SET valrfctran = " & intValor & " " &
                                                  "WHERE datPdp = '" & Format(Session("datEscolhida"), "yyyyMMdd") & "' " &
                                                  "AND codusina = '" & cboUsina.Items(intUsina).Value & "' " &
                                                  "AND intrfc = " & intI
                                Cmd.ExecuteNonQuery()
                                intUsina = intUsina + 1
                ...
                'Grava evento registrando o recebimento de Restrição por Falta de Combustível
                GravaEventoPDP("51", Format(Session("datEscolhida"), "yyyyMMdd"), cboEmpresa.SelectedItem.Text, datAtual, "PDPColResFalCom", UsuarID)
