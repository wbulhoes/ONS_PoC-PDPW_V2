Partial Class frmColEnergetica
...
#End Region
    ...
    Private Sub cboUsina_SelectedIndexChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cboUsina.SelectedIndexChanged
        ...
        Try
            If cboUsina.SelectedItem.Text = "Todas as Usinas" Then
                Cmd.CommandText = "Select d.valrazenertran, " &
                                "       d.intrazener, " &
                                "       d.codusina, " &
                                "       u.ordem " &
                                "From razaoener d, " &
            ...
            If cboUsina.SelectedItem.Text <> "Todas as Usinas" Then
                'objTextArea.Attributes.Item("style") = "font-size:Medium;height:1001px;width:81px;"
                objTextArea.Attributes.Item("style") = "font-size:XSmall;height:1001px;width:81px;line-height:20px"
            Else
                'objTextArea.Attributes.Item("style") = "font-size:Medium;height:1001px;width:" & (((cboUsina.Items.Count - 2) * 65) + 16) - Int(cboUsina.Items.Count / 2) & "px;"
            ...
            Do While rsUsina.Read
                If blnPassou Then
                    If rsUsina("intrazener") <> intInstante Then
                        intInstante = rsUsina("intrazener")
                        objTextArea.Value = objTextArea.Value & Chr(13)
                    Else
                        objTextArea.Value = objTextArea.Value & Chr(9)
                    End If
                End If
                If Not IsDBNull(rsUsina.Item("valrazenertran")) Then
                    objTextArea.Value = objTextArea.Value & rsUsina.Item("valrazenertran")
                Else
    ...
    Private Sub PreencheTable()
        ...
        Try
            ...
            objRow.Controls.Add(objCel)
            tblEnergetica.Width = objTamanho.Pixel(132)
            tblEnergetica.Controls.Add(objRow)
            ...
            Do While rsUsina.Read
                If strCodUsina <> rsUsina.Item("codusina") Then
                    ...
                    objCel.Font.Bold = True
                    tblEnergetica.Rows(0).Width = objTamanho.Pixel(tblEnergetica.Rows(0).Width.Value + 64)
                    tblEnergetica.Width = objTamanho.Pixel(tblEnergetica.Width.Value + 64)
                    ...
                    tblEnergetica.Rows(0).Controls.Add(objCel)
                ...
                objCel.Width = objTamanho.Pixel(64)
                If Not IsDBNull(rsUsina.Item("valrazenertran")) Then
                    objCel.Text = rsUsina.Item("valrazenertran")
                    dblMedia = dblMedia + rsUsina.Item("valrazenertran")
                Else
                    objCel.Text = 0
                End If
                tblEnergetica.Rows(intLin).Width = objTamanho.Pixel(tblEnergetica.Rows(intLin).Width.Value + 63)
                tblEnergetica.Rows(intLin).Controls.Add(objCel)
                intLin = intLin + 1
                If intLin = 49 Then
                    objCel = New TableCell
                    objCel.Wrap = False
                    ...
                    tblEnergetica.Rows(intLin).Width = objTamanho.Pixel(tblEnergetica.Rows(intLin).Width.Value + 64)
                    tblEnergetica.Rows(intLin).Controls.Add(objCel)
                    tblEnergetica.Rows(intLin).Cells(1).Text = Trim(Str(Val(tblEnergetica.Rows(intLin).Cells(1).Text) + dblMedia))  ' (dblMedia / 2)))

                    objCel = New TableCell
                    objCel.Wrap = False
                    objCel.Width = objTamanho.Pixel(64)
                    objCel.Text = Trim(Str(Int(dblMedia / 48)))
                    tblEnergetica.Rows(intLin + 1).Width = objTamanho.Pixel(tblEnergetica.Rows(intLin + 1).Width.Value + 64)
                    tblEnergetica.Rows(intLin + 1).Controls.Add(objCel)
                    tblEnergetica.Rows(intLin + 1).Cells(1).Text = Trim(Str(Val(tblEnergetica.Rows(intLin + 1).Cells(1).Text) + Int(dblMedia / 48)))
            ...
            For intI = 1 To 50
                dblMedia = 0
                For intJ = 2 To tblEnergetica.Rows(0).Cells.Count - 1
                    dblMedia = dblMedia + Val(tblEnergetica.Rows(intI).Cells(intJ).Text)
                Next
                tblEnergetica.Rows(intI).Cells(1).Text = dblMedia '/ 2
            Next
    ...
    Private Sub btnSalvar_Click(ByVal sender As System.Object, ByVal e As System.Web.UI.ImageClickEventArgs) Handles btnSalvar.Click
        ...
        'objPermissao = New OnsClasses.OnsInterfaceUnica.OnsPermissao("UPD", "PDPColEner", UsuarID)
        ...
        If EstaAutorizado Then
            ...
            Try
                ...
                'Grava evento registrando o recebimento de Energetica
                GravaEventoPDP("17", Format(Session("datEscolhida"), "yyyyMMdd"), cboEmpresa.SelectedItem.Text, datAtual, "PDPColEner", UsuarID)
