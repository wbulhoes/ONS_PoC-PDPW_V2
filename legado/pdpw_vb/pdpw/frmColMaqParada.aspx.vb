...
#End Region
    ...
    Private Sub cboEmpresa_SelectedIndexChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cboEmpresa.SelectedIndexChanged
        ...
        Try
            ...
            If Not ValidaLimiteEntradaDados(cboEmpresa.SelectedItem.Value, cboData.SelectedItem.Value, lRetorno, "MCO") Then
                btnSalvar.Visible = False
    ...
    Private Sub cboUsina_SelectedIndexChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cboUsina.SelectedIndexChanged
        ...
        Try
            If cboUsina.SelectedItem.Text = "Todas as Usinas" Then
                Cmd.CommandText = "Select d.valmcotran, " &
                                "       d.intmco, " &
                                "       d.codusina, " &
                                "       u.ordem " &
                                "From conveniencia_oper d, " &
            ...
            Do While rsUsina.Read
                If blnPassou Then
                    If rsUsina("intmco") <> intInstante Then
                        intInstante = rsUsina("intmco")
                        objTextArea.Value = objTextArea.Value & Chr(13)
                    Else
                        objTextArea.Value = objTextArea.Value & Chr(9)
                    End If
                End If
                If Not IsDBNull(rsUsina.Item("valmcotran")) Then
                    objTextArea.Value = objTextArea.Value & rsUsina.Item("valmcotran")
                Else
    ...
    Private Sub PreencheTable()
        ...
        Try
            ...
            objRow.Controls.Add(objCel)
            tblMaqParada.Width = objTamanho.Pixel(132)
            tblMaqParada.Controls.Add(objRow)
            ...
            Do While rsUsina.Read
                If strCodUsina <> rsUsina.Item("codusina") Then
                    ...
                    objCel.Font.Bold = True
                    tblMaqParada.Rows(0).Width = objTamanho.Pixel(tblMaqParada.Rows(0).Width.Value + 64)
                    tblMaqParada.Width = objTamanho.Pixel(tblMaqParada.Width.Value + 64)
                    ...
                    tblMaqParada.Rows(0).Controls.Add(objCel)
                ...
                objCel.Width = objTamanho.Pixel(64)
                If Not IsDBNull(rsUsina.Item("valmcotran")) Then
                    objCel.Text = rsUsina.Item("valmcotran")
                    dblMedia = dblMedia + rsUsina.Item("valmcotran")
                Else
                    objCel.Text = 0
                End If
                tblMaqParada.Rows(intLin).Width = objTamanho.Pixel(tblMaqParada.Rows(intLin).Width.Value + 63)
                tblMaqParada.Rows(intLin).Controls.Add(objCel)
                intLin = intLin + 1
                If intLin = 49 Then
                    objCel = New TableCell
                    objCel.Wrap = False
                    ...
                    tblMaqParada.Rows(intLin).Width = objTamanho.Pixel(tblMaqParada.Rows(intLin).Width.Value + 64)
                    tblMaqParada.Rows(intLin).Controls.Add(objCel)
                    tblMaqParada.Rows(intLin).Cells(1).Text = Trim(Str(Val(tblMaqParada.Rows(intLin).Cells(1).Text) + dblMedia))  ' (dblMedia / 2)))

                    objCel = New TableCell
                    objCel.Wrap = False
                    objCel.Width = objTamanho.Pixel(64)
                    objCel.Text = Trim(Str(Int(dblMedia / 48)))
                    tblMaqParada.Rows(intLin + 1).Width = objTamanho.Pixel(tblMaqParada.Rows(intLin + 1).Width.Value + 64)
                    tblMaqParada.Rows(intLin + 1).Controls.Add(objCel)
                    tblMaqParada.Rows(intLin + 1).Cells(1).Text = Trim(Str(Val(tblMaqParada.Rows(intLin + 1).Cells(1).Text) + Int(dblMedia / 48)))
            ...
            For intI = 1 To 50
                dblMedia = 0
                For intJ = 2 To tblMaqParada.Rows(0).Cells.Count - 1
                    dblMedia = dblMedia + Val(tblMaqParada.Rows(intI).Cells(intJ).Text)
                Next
                tblMaqParada.Rows(intI).Cells(1).Text = dblMedia '/ 2
            Next
    ...
    Private Sub btnSalvar_Click(ByVal sender As System.Object, ByVal e As System.Web.UI.ImageClickEventArgs) Handles btnSalvar.Click
        ...
        objPermissao = New OnsClasses.OnsInterfaceUnica.OnsPermissao("UPD", "PDPColMaqParada", Session("usuarID"))
        ...
        If objPermissao.EstaAutorizado Then
            ...
            Try
                ...
                If strValor = "" Then
                    ...
                    If cboUsina.SelectedItem.Text <> "Todas as Usinas" Then
                        For intI = 1 To 48
                            ...
                            Cmd.CommandText = "Update conveniencia_oper " & _
                                                "Set valmcotran = 0 " & _
                ...
                'Grava evento registrando o recebimento de Número de Máquinas Paradas por Conveniência Operativa
                GravaEventoPDP("36", Format(Session("datEscolhida"), "yyyyMMdd"), cboEmpresa.SelectedItem.Text, datAtual, "PDPColMaqParada", Session("usuarID"))
