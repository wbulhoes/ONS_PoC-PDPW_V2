Partial Class frmColCompensacao
...
#End Region
    ...
    Private Sub cboEmpresa_SelectedIndexChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cboEmpresa.SelectedIndexChanged
        ...
        Try
            ...
            If Not ValidaLimiteEntradaDados(cboEmpresa.SelectedItem.Value, cboData.SelectedItem.Value, lRetorno, "CLF") Then
    ...
    Private Sub cboUsina_SelectedIndexChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cboUsina.SelectedIndexChanged
        ...
        Try
            If cboUsina.SelectedItem.Text = "Todas as Usinas" Then
                Cmd.CommandText = "Select d.valclftran, " &
                                "       d.intclf, " &
                                "       d.codusina, " &
                                "       u.ordem " &
                                "From complastro_fisico d, " &
            ...
            Do While rsUsina.Read
                If blnPassou Then
                    If rsUsina("intclf") <> intInstante Then
                        intInstante = rsUsina("intclf")
                        objTextArea.Value = objTextArea.Value & Chr(13)
                    Else
                        objTextArea.Value = objTextArea.Value & Chr(9)
                    End If
                End If
                If Not IsDBNull(rsUsina.Item("valclftran")) Then
                    objTextArea.Value = objTextArea.Value & rsUsina.Item("valclftran")
                Else
    ...
    Private Sub PreencheTable()
        ...
        Try
            ...
            objRow.Controls.Add(objCel)
            tblCLF.Width = objTamanho.Pixel(132)
            tblCLF.Controls.Add(objRow)
            ...
            Do While rsUsina.Read
                If strCodUsina <> rsUsina.Item("codusina") Then
                    ...
                    objCel.Font.Bold = True
                    tblCLF.Rows(0).Width = objTamanho.Pixel(tblCLF.Rows(0).Width.Value + 64)
                    tblCLF.Width = objTamanho.Pixel(tblCLF.Width.Value + 64)
                    ...
                    tblCLF.Rows(0).Controls.Add(objCel)
                ...
                objCel.Width = objTamanho.Pixel(64)
                If Not IsDBNull(rsUsina.Item("valclftran")) Then
                    objCel.Text = rsUsina.Item("valclftran")
                    dblMedia = dblMedia + rsUsina.Item("valclftran")
                Else
                    objCel.Text = 0
                End If
                tblCLF.Rows(intLin).Width = objTamanho.Pixel(tblCLF.Rows(intLin).Width.Value + 63)
                tblCLF.Rows(intLin).Controls.Add(objCel)
                intLin = intLin + 1
                If intLin = 49 Then
                    objCel = New TableCell
                    objCel.Wrap = False
                    ...
                    tblCLF.Rows(intLin).Width = objTamanho.Pixel(tblCLF.Rows(intLin).Width.Value + 64)
                    tblCLF.Rows(intLin).Controls.Add(objCel)
                    tblCLF.Rows(intLin).Cells(1).Text = Trim(Str(Val(tblCLF.Rows(intLin).Cells(1).Text) + dblMedia)) '(dblMedia / 2)))

                    objCel = New TableCell
                    objCel.Wrap = False
                    objCel.Width = objTamanho.Pixel(64)
                    objCel.Text = Trim(Str(Int(dblMedia / 48)))
                    tblCLF.Rows(intLin + 1).Width = objTamanho.Pixel(tblCLF.Rows(intLin + 1).Width.Value + 64)
                    tblCLF.Rows(intLin + 1).Controls.Add(objCel)
                    tblCLF.Rows(intLin + 1).Cells(1).Text = Trim(Str(Val(tblCLF.Rows(intLin + 1).Cells(1).Text) + Int(dblMedia / 48)))
            ...
            For intI = 1 To 50
                dblMedia = 0
                For intJ = 2 To tblCLF.Rows(0).Cells.Count - 1
                    dblMedia = dblMedia + Val(tblCLF.Rows(intI).Cells(intJ).Text)
                Next
                tblCLF.Rows(intI).Cells(1).Text = dblMedia '/ 2
            Next
            rsUsina.Close()
            rsUsina = Nothing
            'Cmd.Connection.Close()
    ...
    Private Sub btnSalvar_Click(ByVal sender As System.Object, ByVal e As System.Web.UI.ImageClickEventArgs) Handles btnSalvar.Click
        ...
        'objPermissao = New OnsClasses.OnsInterfaceUnica.OnsPermissao("UPD", "PDPColClf", UsuarID)
        ...
        If EstaAutorizado Then
            ...
            Try
                ...
                'Grava evento registrando o recebimento de Compensação de Lastro Físico (CLF)
                GravaEventoPDP("47", Format(Session("datEscolhida"), "yyyyMMdd"), cboEmpresa.SelectedItem.Text, datAtual, "PDPColClf", UsuarID)
