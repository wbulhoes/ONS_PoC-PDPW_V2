Partial Class frmColGerForaMerito
...
#Region " Web Form Designer Generated Code "
    ...
    End Sub
    Protected WithEvents frmColRampa As System.Web.UI.HtmlControls.HtmlForm
...
#End Region
    ...
    Private Sub cboEmpresa_SelectedIndexChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cboEmpresa.SelectedIndexChanged
        ...
        Try
            ...
            If Not ValidaLimiteEntradaDados(cboEmpresa.SelectedItem.Value, cboData.SelectedItem.Value, lRetorno, "GFM") Then
    ...
    Private Sub cboUsina_SelectedIndexChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cboUsina.SelectedIndexChanged
        ...
        Try
            If cboUsina.SelectedItem.Text = "Todas as Usinas" Then
                Cmd.CommandText = "SELECT d.valgfmtran, d.intgfm, d.codusina, u.ordem " &
                                  "FROM tb_gfm d, usina u " &
                                  "WHERE u.codempre = '" & cboEmpresa.SelectedItem.Value & "' " &
                                  "AND u.codusina = d.codusina " &
                                  "AND d.datpdp = '" & Format(Session("datEscolhida"), "yyyyMMdd") & "' " &
                                  "AND u.flg_recebepdpage = 'S' " &
                                  "ORDER BY d.intgfm, u.ordem, d.codusina"
            Else
                Cmd.CommandText = "SELECT d.valgfmtran, d.intgfm " &
                                  "FROM tb_gfm d, usina u " &
                                  "WHERE u.codempre = '" & cboEmpresa.SelectedItem.Value & "' " &
                                  "AND u.codusina = d.codusina " &
            ...
            Do While rsUsina.Read
                If blnPassou Then
                    If rsUsina("intgfm") <> intInstante Then
                        intInstante = rsUsina("intgfm")
                        objTextArea.Value = objTextArea.Value & Chr(13)
                    Else
                        objTextArea.Value = objTextArea.Value & Chr(9)
                    End If
                End If
                If Not IsDBNull(rsUsina.Item("valgfmtran")) Then
                    objTextArea.Value = objTextArea.Value & rsUsina.Item("valgfmtran")
                Else
    ...
    Private Sub PreencheTable()
        ...
        Dim strCodUsina As String
        Try
            Cmd.CommandText = "SELECT d.codusina, d.intgfm, d.valgfmtran, u.ordem " &
                              "FROM tb_gfm d, usina u " &
                              "WHERE u.codempre = '" & Session("strCodEmpre") & "' " &
                              "AND u.codusina = d.codusina " &
                              "AND d.datpdp = '" & Format(Session("datEscolhida"), "yyyyMMdd") & "' " &
                              "AND u.flg_recebepdpage = 'S' " &
                              "ORDER BY u.ordem, d.codusina, d.intgfm "
            Conn.Open("pdp")
            Dim rsUsina As OnsClasses.OnsData.OnsDataReader = Cmd.ExecuteReader
            tblGFM.Rows.Clear()
            ...
            objRow.Controls.Add(objCel)
            tblGFM.Width = objTamanho.Pixel(132)
            tblGFM.Controls.Add(objRow)
            ...
            Do While rsUsina.Read
                If strCodUsina <> rsUsina.Item("codusina") Then
                    ...
                    objCel.Font.Bold = True
                    tblGFM.Rows(0).Width = objTamanho.Pixel(tblGFM.Rows(0).Width.Value + 64)
                    tblGFM.Width = objTamanho.Pixel(tblGFM.Width.Value + 64)
                    ...
                    tblGFM.Rows(0).Controls.Add(objCel)
                ...
                objCel.Width = objTamanho.Pixel(64)
                If Not IsDBNull(rsUsina.Item("valgfmtran")) Then
                    objCel.Text = rsUsina.Item("valgfmtran")
                    dblMedia = dblMedia + rsUsina("valgfmtran")
                Else
                    objCel.Text = 0
                End If
                tblGFM.Rows(intLin).Width = objTamanho.Pixel(tblGFM.Rows(intLin).Width.Value + 63)
                tblGFM.Rows(intLin).Controls.Add(objCel)
                intLin = intLin + 1
                If intLin = 49 Then
                    objCel = New TableCell
                    objCel.Wrap = False
                    ...
                    tblGFM.Rows(intLin).Width = objTamanho.Pixel(tblGFM.Rows(intLin).Width.Value + 64)
                    tblGFM.Rows(intLin).Controls.Add(objCel)
                    tblGFM.Rows(intLin).Cells(1).Text = Trim(Str(Val(tblGFM.Rows(intLin).Cells(1).Text) + dblMedia)) '(dblMedia / 2)))

                    objCel = New TableCell
                    objCel.Wrap = False
                    objCel.Width = objTamanho.Pixel(64)
                    objCel.Text = Trim(Str(Int(dblMedia / 48)))
                    tblGFM.Rows(intLin + 1).Width = objTamanho.Pixel(tblGFM.Rows(intLin + 1).Width.Value + 64)
                    tblGFM.Rows(intLin + 1).Controls.Add(objCel)
                    tblGFM.Rows(intLin + 1).Cells(1).Text = Trim(Str(Val(tblGFM.Rows(intLin + 1).Cells(1).Text) + Int(dblMedia / 48)))
            ...
            For intI = 1 To 50
                dblMedia = 0
                For intJ = 2 To tblGFM.Rows(0).Cells.Count - 1
                    dblMedia = dblMedia + Val(tblGFM.Rows(intI).Cells(intJ).Text)
                Next
                tblGFM.Rows(intI).Cells(1).Text = dblMedia '/ 2
            Next
            rsUsina.Close()
            rsUsina = Nothing
            'Cmd.Connection.Close()
    ...
    Private Sub btnSalvar_Click(ByVal sender As System.Object, ByVal e As System.Web.UI.ImageClickEventArgs) Handles btnSalvar.Click
        ...
        'objPermissao = New OnsClasses.OnsInterfaceUnica.OnsPermissao("UPD", "PDPColGfm", UsuarID)
        ...
        If EstaAutorizado Then
            ...
            Try
                ...
                If strValor = "" Then
                    ...
                    If cboUsina.SelectedItem.Text <> "Todas as Usinas" Then
                        For intI = 1 To 48
                            ...
                            Cmd.CommandText = "UPDATE tb_gfm " & _
                                              "SET valgfmtran = 0 " & _
                ...
                'Grava evento registrando o recebimento de Geração Fora de Mérito (GFM)
                GravaEventoPDP("53", Format(Session("datEscolhida"), "yyyyMMdd"), cboEmpresa.SelectedItem.Text, datAtual, "PDPColGfm", Session("usuarID"))
