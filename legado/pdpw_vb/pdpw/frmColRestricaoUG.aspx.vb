...
Partial Class frmColRestricaoUG
...
#End Region
    ...
    Private Sub Page_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        If Not Page.IsPostBack Then
            ...
            Try
                Cmd.CommandText = "Select codgerad, capgerad " & _
                                "From gerad " & _
                ...
                Do While rsGerador.Read
                    If inti = 1 Then
                        txtValGer.Text = rsGerador("capgerad")
                        inti = inti + 1
                    End If
    ...
    Private Sub btnVoltar_Click(ByVal sender As System.Object, ByVal e As System.Web.UI.ImageClickEventArgs) Handles btnVoltar.Click
        Server.Transfer("frmRestricaoUG.aspx")
    End Sub
    ...
    Private Sub InicializaControles()
        ...
        Try
            Cmd.CommandText = "Select t.codrestr, " &
                                ...
                                "       t.datpdp " &
                                "From   restrgerademp t, " &
                                "       gerad g " &
                                "Where  t.codrestr = '" & Session("strCodRestr") & "' And " &
                                "       t.codgerad = g.codgerad"
            Conn.Open("pdp")
            ...
            Do While rsTempRestr.Read
                ...
                For intI = 0 To cboGerador.Items.Count - 1
                    If Trim(cboGerador.Items.Item(intI).Text) = Trim(rsTempRestr("codgerad")) Then
                ...
                Next
                '--
                txtValGer.Text = IIf(Not IsDBNull(rsTempRestr("capgerad")), rsTempRestr("capgerad"), 0)
    ...
    Private Sub btnSalvar_Click(ByVal sender As System.Object, ByVal e As System.Web.UI.ImageClickEventArgs) Handles btnSalvar.Click
        ...
        If Session("strAcao") = "Alterar" Then
            'objPermissao = New OnsClasses.OnsInterfaceUnica.OnsPermissao("UPD", "PDPColReUG", UsuarID)
        Else
            'objPermissao = New OnsClasses.OnsInterfaceUnica.OnsPermissao("INS", "PDPColReUG", UsuarID)
        End If
        ...
        If EstaAutorizado Then
            ...
            Dim intI As Integer
            Dim strSql, strComand As String
                    ...
                    Try
                        If Session("strAcao") = "Alterar" Then
                            ...
                            Else
                                strSql = "obsrestr = '' "
                            End If
                            Cmd.CommandText = "Update restrgerademp " & _
                                                "Set codgerad = '" & cboGerador.SelectedItem.Value & "', " & _
                        ...
                        Else
                            ...
                            Else
                                strSql = "'') "
                            End If
                            Cmd.CommandText = "Insert Into restrgerademp (" & _
                                                "codgerad, " & _
                                                "datinirestr, " & _
                                                "datfimrestr, " & _
                                                "intinirestr, " & _
                        ...
                        GravaEventoPDP("4", Format(Session("datEscolhida"), "yyyyMMdd"), Session("strSigEmpre"), Now, "PDPColReUG", UsuarID)
                    Catch ex As Exception
                        Session("strMensagem") = "Não foi possível acessar a Base de Dados. " & ex.Message
                        Conn.Close()
                        Response.Redirect("frmMensagem.aspx")
                    End Try
                    Server.Transfer("frmRestricaoUG.aspx")
    ...
    Private Sub cboGerador_SelectedIndexChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cboGerador.SelectedIndexChanged
        Dim Conn As OnsClasses.OnsData.OnsConnection = New OnsClasses.OnsData.OnsConnection
        Dim Cmd As OnsClasses.OnsData.OnsCommand = New OnsClasses.OnsData.OnsCommand
        Cmd.Connection = Conn
        Try
            Cmd.CommandText = "Select capgerad " & _
                                "From gerad " & _
                                "Where codgerad = '" & cboGerador.SelectedItem.Text & "'"
            Conn.Open("pdp")
            Dim rsGerador As OnsClasses.OnsData.OnsDataReader = Cmd.ExecuteReader
            rsGerador.Read()
            txtValGer.Text = rsGerador("capgerad")
