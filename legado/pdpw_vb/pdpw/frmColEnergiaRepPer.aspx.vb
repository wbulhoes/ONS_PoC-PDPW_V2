Partial Class frmColEnergiaRepPer
...
#End Region
    ...
    Private Sub cboEmpresa_SelectedIndexChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cboEmpresa.SelectedIndexChanged
        ...
        Try
            ...
            If Not ValidaLimiteEntradaDados(cboEmpresa.SelectedItem.Value, cboData.SelectedItem.Value, lRetorno, "ERP") Then
    ...
    Private Sub cboUsina_SelectedIndexChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cboUsina.SelectedIndexChanged
        ...
        Try
            If cboUsina.SelectedItem.Text = "Todas as Usinas" Then
                Cmd.CommandText = "Select d.valerptran, " &
                                "       d.interp, " &
                                "       d.codusina, " &
                                "       u.ordem " &
                                "From energia_reposicao d, " &
            ...
            Do While rsUsina.Read
                If blnPassou Then
                    If rsUsina("interp") <> intInstante Then
                        intInstante = rsUsina("interp")
                        objTextArea.Value = objTextArea.Value & Chr(13)
                    Else
                        objTextArea.Value = objTextArea.Value & Chr(9)
                    End If
                End If
                If Not IsDBNull(rsUsina.Item("valerptran")) Then
                    objTextArea.Value = objTextArea.Value & rsUsina.Item("valerptran")
                Else
    ...
    Private Sub PreencheTable()
        ...
        Try
            ...
            objRow.Controls.Add(objCel)
            tblErp.Width = objTamanho.Pixel(132)
            tblErp.Controls.Add(objRow)
            ...
            Do While rsUsina.Read
                If strCodUsina <> rsUsina.Item("codusina") Then
                    ...
                    objCel.Font.Bold = True
                    tblErp.Rows(0).Width = objTamanho.Pixel(tblErp.Rows(0).Width.Value + 64)
                    tblErp.Width = objTamanho.Pixel(tblErp.Width.Value + 64)
                    ...
                    tblErp.Rows(0).Controls.Add(objCel)
                ...
                objCel.Width = objTamanho.Pixel(64)
                If Not IsDBNull(rsUsina.Item("valerptran")) Then
                    objCel.Text = rsUsina.Item("valerptran")
                    dblMedia = dblMedia + rsUsina.Item("valerptran")
                Else
                    objCel.Text = 0
                End If
                tblErp.Rows(intLin).Width = objTamanho.Pixel(tblErp.Rows(intLin).Width.Value + 63)
                tblErp.Rows(intLin).Controls.Add(objCel)
                intLin = intLin + 1
                If intLin = 49 Then
                    objCel = New TableCell
                    objCel.Wrap = False
                    ...
                    tblErp.Rows(intLin).Width = objTamanho.Pixel(tblErp.Rows(intLin).Width.Value + 64)
                    tblErp.Rows(intLin).Controls.Add(objCel)
                    tblErp.Rows(intLin).Cells(1).Text = Trim(Str(Val(tblErp.Rows(intLin).Cells(1).Text) + dblMedia)) '(dblMedia / 2)))

                    objCel = New TableCell
                    objCel.Wrap = False
                    objCel.Width = objTamanho.Pixel(64)
                    objCel.Text = Trim(Str(Int(dblMedia / 48)))
                    tblErp.Rows(intLin + 1).Width = objTamanho.Pixel(tblErp.Rows(intLin + 1).Width.Value + 64)
                    tblErp.Rows(intLin + 1).Controls.Add(objCel)
                    tblErp.Rows(intLin + 1).Cells(1).Text = Trim(Str(Val(tblErp.Rows(intLin + 1).Cells(1).Text) + Int(dblMedia / 48)))
            ...
            For intI = 1 To 50
                dblMedia = 0
                For intJ = 2 To tblErp.Rows(0).Cells.Count - 1
                    dblMedia = dblMedia + Val(tblErp.Rows(intI).Cells(intJ).Text)
                Next
                tblErp.Rows(intI).Cells(1).Text = dblMedia '/ 2
            Next
            rsUsina.Close()
            rsUsina = Nothing
            'Cmd.Connection.Close()
    ...
    Private Sub btnSalvar_Click(ByVal sender As System.Object, ByVal e As System.Web.UI.ImageClickEventArgs) Handles btnSalvar.Click
        ...
        'objPermissao = New OnsClasses.OnsInterfaceUnica.OnsPermissao("UPD", "PDPColErp", UsuarID)
        ...
        If EstaAutorizado Then
            ...
            Try
                ...
                If strValor = "" Then
                    ...
                    If cboUsina.SelectedItem.Text <> "Todas as Usinas" Then
                        For intI = 1 To 48
                            ...
                            Cmd.CommandText = "Update energia_reposicao " & _
                                              "Set valerptran = 0 " & _
                ...
                'Grava evento registrando o recebimento de Energia de Reposição e Perdas (ERP)
                GravaEventoPDP("39", Format(Session("datEscolhida"), "yyyyMMdd"), cboEmpresa.SelectedItem.Text, datAtual, "PDPColErp", UsuarID)
