Public Class frmColModOpTermica
...
#Region " Web Form Designer Generated Code "
    ...
    End Sub
    Protected WithEvents cboData As System.Web.UI.WebControls.DropDownList
    Protected WithEvents cboEmpresa As System.Web.UI.WebControls.DropDownList
    Protected WithEvents cboUsina As System.Web.UI.WebControls.DropDownList
    Protected WithEvents btnSalvar As System.Web.UI.WebControls.ImageButton
    Protected WithEvents divValor As System.Web.UI.HtmlControls.HtmlGenericControl
    Protected WithEvents tbltb_mot As System.Web.UI.WebControls.Table
    Protected WithEvents tblmotivoinfl As System.Web.UI.WebControls.Table
...
#End Region
    ...
    Private Sub cboUsina_SelectedIndexChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cboUsina.SelectedIndexChanged
        ...
        Try
            If cboUsina.SelectedItem.Text = "Todas as Usinas" Then
                Cmd.CommandText = "SELECT d.valmottran, d.intmot, d.codusina, u.ordem " &
                                  "FROM tb_mot d, usina u " &
                                  ...
                                  "AND d.datpdp = '" & Format(Session("datEscolhida"), "yyyyMMdd") & "' " &
                                  "AND u.flg_recebepdpage = 'S' " &
                                  "ORDER BY d.intmot, u.ordem, d.codusina"
            Else
                Cmd.CommandText = "SELECT d.valmottran, d.intmot " &
                                  "FROM tb_mot d, usina u " &
                                  "WHERE u.codempre = '" & cboEmpresa.SelectedItem.Value & "' " &
                                  "AND u.tipusina = 'T' " &
                                  "AND u.codusina = d.codusina " &
            ...
            Do While rsUsina.Read
                If blnPassou Then
                    If rsUsina("intmot") <> intInstante Then
                        intInstante = rsUsina("intmot")
                        objTextArea.Value = objTextArea.Value & Chr(13)
                    Else
                        objTextArea.Value = objTextArea.Value & Chr(9)
                    End If
                End If
                If Not IsDBNull(rsUsina.Item("valmottran")) Then
                    objTextArea.Value = objTextArea.Value & rsUsina.Item("valmottran")
                Else
    ...
    Private Sub PreencheTable()
        ...
        Dim strCodUsina As String
        Try
            Cmd.CommandText = "SELECT d.codusina, d.intmot, d.valmottran, u.ordem " &
                              "FROM tb_mot d, usina u " &
                              "WHERE u.codempre = '" & Session("strCodEmpre") & "' " &
                              "AND u.tipusina = 'T' " &
                              "AND u.codusina = d.codusina " &
                              "AND d.datpdp = '" & Format(Session("datEscolhida"), "yyyyMMdd") & "' " &
                              "AND u.flg_recebepdpage = 'S' " &
                              "ORDER BY u.ordem, d.codusina, d.intmot"
            Conn.Open("pdp")
            Dim rsUsina As OnsClasses.OnsData.OnsDataReader = Cmd.ExecuteReader
            tbltb_mot.Rows.Clear()
            ...
            tbltb_mot.Width = objTamanho.Pixel(70)
            tbltb_mot.Controls.Add(objRow)
            ...
            Do While rsUsina.Read
                If strCodUsina <> rsUsina.Item("codusina") Then
                    ...
                    objCel.Font.Bold = True
                    tbltb_mot.Rows(0).Width = objTamanho.Pixel(tbltb_mot.Rows(0).Width.Value + 64)
                    tbltb_mot.Width = objTamanho.Pixel(tbltb_mot.Width.Value + 64)
                    ...
                    tbltb_mot.Rows(0).Controls.Add(objCel)
                ...
                objCel.Width = objTamanho.Pixel(64)
                If Not IsDBNull(rsUsina.Item("valmottran")) Then
                    objCel.Text = rsUsina.Item("valmottran")
                    dblMedia = dblMedia + rsUsina.Item("valmottran")
                Else
                    objCel.Text = 0
                End If
                tbltb_mot.Rows(intLin).Width = objTamanho.Pixel(tbltb_mot.Rows(intLin).Width.Value + 63)
                tbltb_mot.Rows(intLin).Controls.Add(objCel)
                intLin = intLin + 1
            Loop
            rsUsina.Close()
            rsUsina = Nothing
    ...
    Private Sub btnSalvar_Click(ByVal sender As System.Object, ByVal e As System.Web.UI.ImageClickEventArgs) Handles btnSalvar.Click
        ...
        'objPermissao = New OnsClasses.OnsInterfaceUnica.OnsPermissao("UPD", "PDPColMot", UsuarID)
        ...
        If EstaAutorizado Then
            ...
            Try
                ...
                If strValor = "" Then
                    ...
                    If cboUsina.SelectedItem.Text <> "Todas as Usinas" Then
                        For intI = 1 To 48
                            ...
                            Cmd.CommandText = "UPDATE tb_mot " & _
                                              "SET valmottran = 0 " & _
                ...
                'Grava evento registrando o recebimento de Modalidade de Operação das Térmicas
                GravaEventoPDP("56", Format(Session("datEscolhida"), "yyyyMMdd"), cboEmpresa.SelectedItem.Text, datAtual, "PDPColMot", UsuarID)
