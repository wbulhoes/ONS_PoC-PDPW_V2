Partial Class frmColMaqOperando
...
#End Region
    ...
    Private Sub cboEmpresa_SelectedIndexChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cboEmpresa.SelectedIndexChanged
        ...
        Try
            ...
            If Not ValidaLimiteEntradaDados(cboEmpresa.SelectedItem.Value, cboData.SelectedItem.Value, lRetorno, "MOS") Then
                btnSalvar.Visible = False
    ...
    Private Sub cboUsina_SelectedIndexChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cboUsina.SelectedIndexChanged
        ...
        Try
            If cboUsina.SelectedItem.Text = "Todas as Usinas" Then
                Cmd.CommandText = "Select d.valmostran, " &
                                "       d.intmos, " &
                                "       d.codusina, " &
                                "       u.ordem " &
                                "From oper_sincrono d, " &
            ...
            Do While rsUsina.Read
                If blnPassou Then
                    If rsUsina("intmos") <> intInstante Then
                        intInstante = rsUsina("intmos")
                        objTextArea.Value = objTextArea.Value & Chr(13)
                    Else
                        objTextArea.Value = objTextArea.Value & Chr(9)
                    End If
                End If
                If Not IsDBNull(rsUsina.Item("valmostran")) Then
                    objTextArea.Value = objTextArea.Value & rsUsina.Item("valmostran")
                Else
    ...
    Private Sub PreencheTable()
        ...
        Try
            ...
            objRow.Controls.Add(objCel)
            tblMaqOperando.Width = objTamanho.Pixel(132)
            tblMaqOperando.Controls.Add(objRow)
            ...
            Do While rsUsina.Read
                If strCodUsina <> rsUsina.Item("codusina") Then
                    ...
                    objCel.Font.Bold = True
                    tblMaqOperando.Rows(0).Width = objTamanho.Pixel(tblMaqOperando.Rows(0).Width.Value + 64)
                    tblMaqOperando.Width = objTamanho.Pixel(tblMaqOperando.Width.Value + 64)
                    ...
                    tblMaqOperando.Rows(0).Controls.Add(objCel)
                ...
                objCel.Width = objTamanho.Pixel(64)
                If Not IsDBNull(rsUsina.Item("valmostran")) Then
                    objCel.Text = rsUsina.Item("valmostran")
                    dblMedia = dblMedia + rsUsina.Item("valmostran")
                Else
                    objCel.Text = 0
                End If
                tblMaqOperando.Rows(intLin).Width = objTamanho.Pixel(tblMaqOperando.Rows(intLin).Width.Value + 63)
                tblMaqOperando.Rows(intLin).Controls.Add(objCel)
                intLin = intLin + 1
                If intLin = 49 Then
                    objCel = New TableCell
                    objCel.Wrap = False
                    ...
                    tblMaqOperando.Rows(intLin).Width = objTamanho.Pixel(tblMaqOperando.Rows(intLin).Width.Value + 64)
                    tblMaqOperando.Rows(intLin).Controls.Add(objCel)
                    tblMaqOperando.Rows(intLin).Cells(1).Text = Trim(Str(Val(tblMaqOperando.Rows(intLin).Cells(1).Text) + dblMedia))  ' (dblMedia / 2)))

                    objCel = New TableCell
                    objCel.Wrap = False
                    objCel.Width = objTamanho.Pixel(64)
                    objCel.Text = Trim(Str(Int(dblMedia / 48)))
                    tblMaqOperando.Rows(intLin + 1).Width = objTamanho.Pixel(tblMaqOperando.Rows(intLin + 1).Width.Value + 64)
                    tblMaqOperando.Rows(intLin + 1).Controls.Add(objCel)
                    tblMaqOperando.Rows(intLin + 1).Cells(1).Text = Trim(Str(Val(tblMaqOperando.Rows(intLin + 1).Cells(1).Text) + Int(dblMedia / 48)))
            ...
            For intI = 1 To 50
                dblMedia = 0
                For intJ = 2 To tblMaqOperando.Rows(0).Cells.Count - 1
                    dblMedia = dblMedia + Val(tblMaqOperando.Rows(intI).Cells(intJ).Text)
                Next
                tblMaqOperando.Rows(intI).Cells(1).Text = dblMedia '/ 2
            Next
    ...
    Private Sub btnSalvar_Click(ByVal sender As System.Object, ByVal e As System.Web.UI.ImageClickEventArgs) Handles btnSalvar.Click
        ...
        objPermissao = New OnsClasses.OnsInterfaceUnica.OnsPermissao("UPD", "PDPColMaqOperan", Session("usuarID"))
        ...
        If objPermissao.EstaAutorizado Then
            ...
            Try
                ...
                If strValor = "" Then
                    ...
                    If cboUsina.SelectedItem.Text <> "Todas as Usinas" Then
                        For intI = 1 To 48
                            ...
                            Cmd.CommandText = "Update oper_sincrono " & _
                                                "Set valmostran = 0 " & _
                ...
                'Grava evento registrando o recebimento de Número de Máquinas Operando como Síncrono
                GravaEventoPDP("37", Format(Session("datEscolhida"), "yyyyMMdd"), cboEmpresa.SelectedItem.Text, datAtual, "PDPColMaqOperan", Session("usuarID"))
