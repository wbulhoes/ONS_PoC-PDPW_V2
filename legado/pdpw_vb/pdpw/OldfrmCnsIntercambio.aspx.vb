Partial Class frmCnsIntercambio
...
#End Region
    ...
    Private Sub ConsultaIntervalo(ByVal strcampo As String)
        ...
        Try
            Cmd.CommandText = "SELECT codemprede || '-' || codemprepara || '/' || codcontamodal AS codemprepara, intinter, Sum(valinter" & strcampo & ") AS valor " &
                              "FROM inter " &
                              "WHERE codemprede = '" & cboEmpresa.SelectedItem.Value & "' " &
                              "AND datpdp = " & cboDataInicial.SelectedItem.Value & " " &
                              "GROUP BY 1, 2 " &
                              "ORDER BY 1, 2"
            Conn.Open(strBase)
            ...
            'nova Celula
            tblConsulta.Width = objTamanho.Pixel(200)
            intTamanho = 200
            objRow = New TableRow
            ...
            Do While rsConsulta.Read
                If strCodEmpresa <> rsConsulta("codemprepara") Then
                    If intI <> 0 Then
                        objCell = New TableCell
                        objCell.Font.Bold = True
                    ...
                    objCell.Text = rsConsulta("codemprepara")
                    tblConsulta.Rows(0).Controls.Add(objCell)
                    strCodEmpresa = rsConsulta("codemprepara")
                    intI = 1
                End If
