...
#End Region
    ...
    Private Sub cboEmpresa_SelectedIndexChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cboEmpresa.SelectedIndexChanged
        ...
        Try
            ...
            If Not ValidaLimiteEntradaDados(cboEmpresa.SelectedItem.Value, cboData.SelectedItem.Value, lRetorno, "DCR") Then
    ...
    Private Sub cboUsina_SelectedIndexChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cboUsina.SelectedIndexChanged
        ...
        Try
            If cboUsina.SelectedItem.Text = "Todas as Usinas" Then
                Cmd.CommandText = "Select d.valdcrtran, " &
                                "       d.intdcr, " &
                                "       d.codusina, " &
                                "       u.ordem " &
                                "From tb_DCR d, " &
            ...
            Do While rsUsina.Read
                If blnPassou Then
                    If rsUsina("intdcr") <> intInstante Then
                        intInstante = rsUsina("intdcr")
                        objTextArea.Value = objTextArea.Value & Chr(13)
                    Else
                        objTextArea.Value = objTextArea.Value & Chr(9)
                    End If
                End If
                If Not IsDBNull(rsUsina.Item("valdcrtran")) Then
                    objTextArea.Value = objTextArea.Value & rsUsina.Item("valdcrtran")
                Else
    ...
    Private Sub PreencheTable()
        ...
        Try
            ...
            objRow.Controls.Add(objCel)
            tblDCR.Width = objTamanho.Pixel(132)
            tblDCR.Controls.Add(objRow)
            ...
            Do While rsUsina.Read
                If strCodUsina <> rsUsina.Item("codusina") Then
                    ...
                    objCel.Font.Bold = True
                    tblDCR.Rows(0).Width = objTamanho.Pixel(tblDCR.Rows(0).Width.Value + 64)
                    tblDCR.Width = objTamanho.Pixel(tblDCR.Width.Value + 64)
                    ...
                    tblDCR.Rows(0).Controls.Add(objCel)
                ...
                objCel.Width = objTamanho.Pixel(64)
                If Not IsDBNull(rsUsina.Item("valdcrtran")) Then
                    objCel.Text = rsUsina.Item("valdcrtran")
                    dblMedia = dblMedia + rsUsina.Item("valdcrtran")
                Else
                    objCel.Text = 0
                End If
                tblDCR.Rows(intLin).Width = objTamanho.Pixel(tblDCR.Rows(intLin).Width.Value + 63)
                tblDCR.Rows(intLin).Controls.Add(objCel)
                intLin = intLin + 1
                If intLin = 49 Then
                    objCel = New TableCell
                    objCel.Wrap = False
                    ...
                    tblDCR.Rows(intLin).Width = objTamanho.Pixel(tblDCR.Rows(intLin).Width.Value + 64)
                    tblDCR.Rows(intLin).Controls.Add(objCel)
                    tblDCR.Rows(intLin).Cells(1).Text = Trim(Str(Val(tblDCR.Rows(intLin).Cells(1).Text) + dblMedia)) '(dblMedia / 2)))

                    objCel = New TableCell
                    objCel.Wrap = False
                    objCel.Width = objTamanho.Pixel(64)
                    objCel.Text = Trim(Str(Int(dblMedia / 48)))
                    tblDCR.Rows(intLin + 1).Width = objTamanho.Pixel(tblDCR.Rows(intLin + 1).Width.Value + 64)
                    tblDCR.Rows(intLin + 1).Controls.Add(objCel)
                    tblDCR.Rows(intLin + 1).Cells(1).Text = Trim(Str(Val(tblDCR.Rows(intLin + 1).Cells(1).Text) + Int(dblMedia / 48)))
            ...
            For intI = 1 To 50
                dblMedia = 0
                For intJ = 2 To tblDCR.Rows(0).Cells.Count - 1
                    dblMedia = dblMedia + Val(tblDCR.Rows(intI).Cells(intJ).Text)
                Next
                tblDCR.Rows(intI).Cells(1).Text = dblMedia '/ 2
            Next
            rsUsina.Close()
            rsUsina = Nothing
            'Cmd.Connection.Close()
    ...
    Private Sub btnSalvar_Click(ByVal sender As System.Object, ByVal e As System.Web.UI.ImageClickEventArgs) Handles btnSalvar.Click
        ...
        If EstaAutorizado Then
            ...
            Try
                ...
                'Grava evento registrando o recebimento de DCR (DSP)
