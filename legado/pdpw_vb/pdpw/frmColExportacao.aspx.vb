Partial Class frmColExportacao
...
#End Region
    ...
    Private Sub cboEmpresa_SelectedIndexChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cboEmpresa.SelectedIndexChanged
        ...
        Try
            ...
            If Not ValidaLimiteEntradaDados(cboEmpresa.SelectedItem.Value, cboData.SelectedItem.Value, lRetorno, "EXP") Then
    ...
    Private Sub cboUsina_SelectedIndexChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cboUsina.SelectedIndexChanged
        ...
        Try
            If cboUsina.SelectedItem.Text = "Todas as Usinas" Then
                Cmd.CommandText = "Select d.valexportatran, " &
                                "       d.intexporta, " &
                                "       d.codusina, " &
                                "       u.ordem " &
                                "From exporta d, " &
            ...
            Do While rsUsina.Read
                If blnPassou Then
                    If rsUsina("intexporta") <> intInstante Then
                        intInstante = rsUsina("intexporta")
                        objTextArea.Value = objTextArea.Value & Chr(13)
                    Else
                        objTextArea.Value = objTextArea.Value & Chr(9)
                    End If
                End If
                If Not IsDBNull(rsUsina.Item("valexportatran")) Then
                    objTextArea.Value = objTextArea.Value & rsUsina.Item("valexportatran")
                Else
    ...
    Private Sub PreencheTable()
        ...
        Try
            ...
            objRow.Controls.Add(objCel)
            tblExportacao.Width = objTamanho.Pixel(132)
            tblExportacao.Controls.Add(objRow)
            ...
            Do While rsUsina.Read
                If strCodUsina <> rsUsina.Item("codusina") Then
                    ...
                    objCel.Font.Bold = True
                    tblExportacao.Rows(0).Width = objTamanho.Pixel(tblExportacao.Rows(0).Width.Value + 64)
                    tblExportacao.Width = objTamanho.Pixel(tblExportacao.Width.Value + 64)
                    ...
                    tblExportacao.Rows(0).Controls.Add(objCel)
                ...
                objCel.Width = objTamanho.Pixel(64)
                If Not IsDBNull(rsUsina.Item("valexportatran")) Then
                    objCel.Text = rsUsina.Item("valexportatran")
                    dblMedia = dblMedia + rsUsina.Item("valexportatran")
                Else
                    objCel.Text = 0
                End If
                tblExportacao.Rows(intLin).Width = objTamanho.Pixel(tblExportacao.Rows(intLin).Width.Value + 63)
                tblExportacao.Rows(intLin).Controls.Add(objCel)
                intLin = intLin + 1
                If intLin = 49 Then
                    objCel = New TableCell
                    objCel.Wrap = False
                    ...
                    tblExportacao.Rows(intLin).Width = objTamanho.Pixel(tblExportacao.Rows(intLin).Width.Value + 64)
                    tblExportacao.Rows(intLin).Controls.Add(objCel)
                    tblExportacao.Rows(intLin).Cells(1).Text = Trim(Str(Val(tblExportacao.Rows(intLin).Cells(1).Text) + dblMedia)) '(dblMedia / 2)))

                    objCel = New TableCell
                    objCel.Wrap = False
                    objCel.Width = objTamanho.Pixel(64)
                    objCel.Text = Trim(Str(Int(dblMedia / 48)))
                    tblExportacao.Rows(intLin + 1).Width = objTamanho.Pixel(tblExportacao.Rows(intLin + 1).Width.Value + 64)
                    tblExportacao.Rows(intLin + 1).Controls.Add(objCel)
                    tblExportacao.Rows(intLin + 1).Cells(1).Text = Trim(Str(Val(tblExportacao.Rows(intLin + 1).Cells(1).Text) + Int(dblMedia / 48)))
            ...
            For intI = 1 To 50
                dblMedia = 0
                For intJ = 2 To tblExportacao.Rows(0).Cells.Count - 1
                    dblMedia = dblMedia + Val(tblExportacao.Rows(intI).Cells(intJ).Text)
                Next
                tblExportacao.Rows(intI).Cells(1).Text = dblMedia '/ 2
            Next
    ...
    Private Sub btnSalvar_Click(ByVal sender As System.Object, ByVal e As System.Web.UI.ImageClickEventArgs) Handles btnSalvar.Click
        ...
        'objPermissao = New OnsClasses.OnsInterfaceUnica.OnsPermissao("UPD", "PDPColExporta", UsuarID)
        ...
        If EstaAutorizado Then
            ...
            Try
                ...
                If strValor = "" Then
                    ...
                    If cboUsina.SelectedItem.Text <> "Todas as Usinas" Then
                        For intI = 1 To 48
                            ...
                            Cmd.CommandText = "Update exporta " & _
                                                "Set valexportatran = 0 " & _
                ...
                'Grava evento registrando o recebimento de exportaçãp
                GravaEventoPDP("33", Format(Session("datEscolhida"), "yyyyMMdd"), cboEmpresa.SelectedItem.Text, datAtual, "PDPColExporta", UsuarID)
