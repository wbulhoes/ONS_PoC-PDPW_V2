...
#End Region
    ...
    Private Sub cboEmpresa_SelectedIndexChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cboEmpresa.SelectedIndexChanged
        ...
        Try
            ...
            If Not ValidaLimiteEntradaDados(cboEmpresa.SelectedItem.Value, cboData.SelectedItem.Value, lRetorno, "DCA") Then
    ...
    Private Sub cboUsina_SelectedIndexChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cboUsina.SelectedIndexChanged
        ...
        Try
            If cboUsina.SelectedItem.Text = "Todas as Usinas" Then
                Cmd.CommandText = "Select d.valdcatran, " &
                                "       d.intdca, " &
                                "       d.codusina, " &
                                "       u.ordem " &
                                "From tb_DCA d, " &
            ...
            Do While rsUsina.Read
                If blnPassou Then
                    If rsUsina("intdca") <> intInstante Then
                        intInstante = rsUsina("intdca")
                        objTextArea.Value = objTextArea.Value & Chr(13)
                    Else
                        objTextArea.Value = objTextArea.Value & Chr(9)
                    End If
                End If
                If Not IsDBNull(rsUsina.Item("valdcatran")) Then
                    objTextArea.Value = objTextArea.Value & rsUsina.Item("valdcatran")
                Else
    ...
    Private Sub PreencheTable()
        ...
        Try
            ...
            objRow.Controls.Add(objCel)
            tblDCA.Width = objTamanho.Pixel(132)
            tblDCA.Controls.Add(objRow)
            ...
            Do While rsUsina.Read
                If strCodUsina <> rsUsina.Item("codusina") Then
                    ...
                    objCel.Font.Bold = True
                    tblDCA.Rows(0).Width = objTamanho.Pixel(tblDCA.Rows(0).Width.Value + 64)
                    tblDCA.Width = objTamanho.Pixel(tblDCA.Width.Value + 64)
                    ...
                    tblDCA.Rows(0).Controls.Add(objCel)
                ...
                objCel.Width = objTamanho.Pixel(64)
                If Not IsDBNull(rsUsina.Item("valdcatran")) Then
                    objCel.Text = rsUsina.Item("valdcatran")
                    dblMedia = dblMedia + rsUsina.Item("valdcatran")
                Else
                    objCel.Text = 0
                End If
                tblDCA.Rows(intLin).Width = objTamanho.Pixel(tblDCA.Rows(intLin).Width.Value + 63)
                tblDCA.Rows(intLin).Controls.Add(objCel)
                intLin = intLin + 1
                If intLin = 49 Then
                    objCel = New TableCell
                    objCel.Wrap = False
                    ...
                    tblDCA.Rows(intLin).Width = objTamanho.Pixel(tblDCA.Rows(intLin).Width.Value + 64)
                    tblDCA.Rows(intLin).Controls.Add(objCel)
                    tblDCA.Rows(intLin).Cells(1).Text = Trim(Str(Val(tblDCA.Rows(intLin).Cells(1).Text) + dblMedia)) '(dblMedia / 2)))

                    objCel = New TableCell
                    objCel.Wrap = False
                    objCel.Width = objTamanho.Pixel(64)
                    objCel.Text = Trim(Str(Int(dblMedia / 48)))
                    tblDCA.Rows(intLin + 1).Width = objTamanho.Pixel(tblDCA.Rows(intLin + 1).Width.Value + 64)
                    tblDCA.Rows(intLin + 1).Controls.Add(objCel)
                    tblDCA.Rows(intLin + 1).Cells(1).Text = Trim(Str(Val(tblDCA.Rows(intLin + 1).Cells(1).Text) + Int(dblMedia / 48)))
            ...
            For intI = 1 To 50
                dblMedia = 0
                For intJ = 2 To tblDCA.Rows(0).Cells.Count - 1
                    dblMedia = dblMedia + Val(tblDCA.Rows(intI).Cells(intJ).Text)
                Next
                tblDCA.Rows(intI).Cells(1).Text = dblMedia '/ 2
            Next
            rsUsina.Close()
            rsUsina = Nothing
            'Cmd.Connection.Close()
    ...
    Private Sub btnSalvar_Click(ByVal sender As System.Object, ByVal e As System.Web.UI.ImageClickEventArgs) Handles btnSalvar.Click
        ...
        If EstaAutorizado Then
            ...
            Try
                ...
                'Grava evento registrando o recebimento de DCA (DSP)
