...
#End Region
    ...
    Private Sub cboEmpresa_SelectedIndexChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cboEmpresa.SelectedIndexChanged
        ...
        Try
            ...
            If Not ValidaLimiteEntradaDados(cboEmpresa.SelectedItem.Value, cboData.SelectedItem.Value, lRetorno, "GES") Then
    ...
    Private Sub cboUsina_SelectedIndexChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cboUsina.SelectedIndexChanged
        ...
        Try
            If cboUsina.SelectedItem.Text = "Todas as Usinas" Then
                Cmd.CommandText = "Select d.valgestran, " &
                                "       d.intges, " &
                                "       d.codusina, " &
                                "       u.ordem " &
                                "From tb_GES d, " &
            ...
            Do While rsUsina.Read
                If blnPassou Then
                    If rsUsina("intges") <> intInstante Then
                        intInstante = rsUsina("intges")
                        objTextArea.Value = objTextArea.Value & Chr(13)
                    Else
                        objTextArea.Value = objTextArea.Value & Chr(9)
                    End If
                End If
                If Not IsDBNull(rsUsina.Item("valgestran")) Then
                    objTextArea.Value = objTextArea.Value & rsUsina.Item("valgestran")
                Else
    ...
    Private Sub PreencheTable()
        ...
        Try
            ...
            objRow.Controls.Add(objCel)
            tblGES.Width = objTamanho.Pixel(132)
            tblGES.Controls.Add(objRow)
            ...
            Do While rsUsina.Read
                If strCodUsina <> rsUsina.Item("codusina") Then
                    ...
                    objCel.Font.Bold = True
                    tblGES.Rows(0).Width = objTamanho.Pixel(tblGES.Rows(0).Width.Value + 64)
                    tblGES.Width = objTamanho.Pixel(tblGES.Width.Value + 64)
                    ...
                    tblGES.Rows(0).Controls.Add(objCel)
                ...
                objCel.Width = objTamanho.Pixel(64)
                If Not IsDBNull(rsUsina.Item("valgestran")) Then
                    objCel.Text = rsUsina.Item("valgestran")
                    dblMedia = dblMedia + rsUsina.Item("valgestran")
                Else
                    objCel.Text = 0
                End If
                tblGES.Rows(intLin).Width = objTamanho.Pixel(tblGES.Rows(intLin).Width.Value + 63)
                tblGES.Rows(intLin).Controls.Add(objCel)
                intLin = intLin + 1
                If intLin = 49 Then
                    objCel = New TableCell
                    objCel.Wrap = False
                    ...
                    tblGES.Rows(intLin).Width = objTamanho.Pixel(tblGES.Rows(intLin).Width.Value + 64)
                    tblGES.Rows(intLin).Controls.Add(objCel)
                    tblGES.Rows(intLin).Cells(1).Text = Trim(Str(Val(tblGES.Rows(intLin).Cells(1).Text) + dblMedia)) '(dblMedia / 2)))

                    objCel = New TableCell
                    objCel.Wrap = False
                    objCel.Width = objTamanho.Pixel(64)
                    objCel.Text = Trim(Str(Int(dblMedia / 48)))
                    tblGES.Rows(intLin + 1).Width = objTamanho.Pixel(tblGES.Rows(intLin + 1).Width.Value + 64)
                    tblGES.Rows(intLin + 1).Controls.Add(objCel)
                    tblGES.Rows(intLin + 1).Cells(1).Text = Trim(Str(Val(tblGES.Rows(intLin + 1).Cells(1).Text) + Int(dblMedia / 48)))
            ...
            For intI = 1 To 50
                dblMedia = 0
                For intJ = 2 To tblGES.Rows(0).Cells.Count - 1
                    dblMedia = dblMedia + Val(tblGES.Rows(intI).Cells(intJ).Text)
                Next
                tblGES.Rows(intI).Cells(1).Text = dblMedia '/ 2
            Next
            rsUsina.Close()
            rsUsina = Nothing
            'Cmd.Connection.Close()
    ...
    Private Sub btnSalvar_Click(ByVal sender As System.Object, ByVal e As System.Web.UI.ImageClickEventArgs) Handles btnSalvar.Click
        ...
        If EstaAutorizado Then
            ...
            Try
                ...
                'Grava evento registrando o recebimento de GES (DSP)
