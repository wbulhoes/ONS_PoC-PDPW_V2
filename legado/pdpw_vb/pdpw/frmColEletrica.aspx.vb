...
#End Region
    ...
    Private Sub cboEmpresa_SelectedIndexChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cboEmpresa.SelectedIndexChanged
        ...
        Try
            ...
            If Not ValidaLimiteEntradaDados(cboEmpresa.SelectedItem.Value, cboData.SelectedItem.Value, lRetorno, "ZEN") Then
    ...
    Private Sub cboUsina_SelectedIndexChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cboUsina.SelectedIndexChanged
        ...
        Try
            If cboUsina.SelectedItem.Text = "Todas as Usinas" Then
                Cmd.CommandText = "Select d.valrazelettran, " &
                                "       d.intrazelet, " &
                                "       d.codusina, " &
                                "       u.ordem " &
                                "From razaoelet d, " &
            ...
            Do While rsUsina.Read
                If blnPassou Then
                    If rsUsina("intrazelet") <> intInstante Then
                        intInstante = rsUsina("intrazelet")
                        objTextArea.Value = objTextArea.Value & Chr(13)
                    Else
                        objTextArea.Value = objTextArea.Value & Chr(9)
                    End If
                End If
                If Not IsDBNull(rsUsina.Item("valrazelettran")) Then
                    objTextArea.Value = objTextArea.Value & rsUsina.Item("valrazelettran")
                Else
    ...
    Private Sub PreencheTable()
        ...
        Try
            ...
            objRow.Controls.Add(objCel)
            tblEletrica.Width = objTamanho.Pixel(132)
            tblEletrica.Controls.Add(objRow)
            ...
            Do While rsUsina.Read
                If strCodUsina <> rsUsina.Item("codusina") Then
                    ...
                    objCel.Font.Bold = True
                    tblEletrica.Rows(0).Width = objTamanho.Pixel(tblEletrica.Rows(0).Width.Value + 64)
                    tblEletrica.Width = objTamanho.Pixel(tblEletrica.Width.Value + 64)
                    ...
                    tblEletrica.Rows(0).Controls.Add(objCel)
                ...
                objCel.Width = objTamanho.Pixel(64)
                If Not IsDBNull(rsUsina.Item("valrazelettran")) Then
                    objCel.Text = rsUsina.Item("valrazelettran")
                    dblMedia = dblMedia + rsUsina.Item("valrazelettran")
                Else
                    objCel.Text = 0
                End If
                tblEletrica.Rows(intLin).Width = objTamanho.Pixel(tblEletrica.Rows(intLin).Width.Value + 63)
                tblEletrica.Rows(intLin).Controls.Add(objCel)
                intLin = intLin + 1
                If intLin = 49 Then
                    objCel = New TableCell
                    objCel.Wrap = False
                    ...
                    tblEletrica.Rows(intLin).Width = objTamanho.Pixel(tblEletrica.Rows(intLin).Width.Value + 64)
                    tblEletrica.Rows(intLin).Controls.Add(objCel)
                    tblEletrica.Rows(intLin).Cells(1).Text = Trim(Str(Val(tblEletrica.Rows(intLin).Cells(1).Text) + dblMedia)) '(dblMedia / 2)))

                    objCel = New TableCell
                    objCel.Wrap = False
                    objCel.Width = objTamanho.Pixel(64)
                    objCel.Text = Trim(Str(Int(dblMedia / 48)))
                    tblEletrica.Rows(intLin + 1).Width = objTamanho.Pixel(tblEletrica.Rows(intLin + 1).Width.Value + 64)
                    tblEletrica.Rows(intLin + 1).Controls.Add(objCel)
                    tblEletrica.Rows(intLin + 1).Cells(1).Text = Trim(Str(Val(tblEletrica.Rows(intLin + 1).Cells(1).Text) + Int(dblMedia / 48)))
            ...
            For intI = 1 To 50
                dblMedia = 0
                For intJ = 2 To tblEletrica.Rows(0).Cells.Count - 1
                    dblMedia = dblMedia + Val(tblEletrica.Rows(intI).Cells(intJ).Text)
                Next
                tblEletrica.Rows(intI).Cells(1).Text = dblMedia '/ 2
            Next
    ...
    Private Sub btnSalvar_Click(ByVal sender As System.Object, ByVal e As System.Web.UI.ImageClickEventArgs) Handles btnSalvar.Click
        ...
        'objPermissao = New OnsClasses.OnsInterfaceUnica.OnsPermissao("UPD", "PDPColElet", UsuarID)
        ...
        If EstaAutorizado Then
            ...
            Try
                ...
                'Grava evento registrando o recebimento de Eletrica
                GravaEventoPDP("18", Format(Session("datEscolhida"), "yyyyMMdd"), cboEmpresa.SelectedItem.Text, datAtual, "PDPColElet", UsuarID)
