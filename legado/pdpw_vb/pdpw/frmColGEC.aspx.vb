...
#End Region
    ...
    Private Sub cboEmpresa_SelectedIndexChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cboEmpresa.SelectedIndexChanged
        ...
        Try
            ...
            If Not ValidaLimiteEntradaDados(cboEmpresa.SelectedItem.Value, cboData.SelectedItem.Value, lRetorno, "GEC") Then
    ...
    Private Sub cboUsina_SelectedIndexChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cboUsina.SelectedIndexChanged
        ...
        Try
            If cboUsina.SelectedItem.Text = "Todas as Usinas" Then
                Cmd.CommandText = "Select d.valgectran, " &
                                "       d.intgec, " &
                                "       d.codusina, " &
                                "       u.ordem " &
                                "From tb_GEC d, " &
            ...
            Do While rsUsina.Read
                If blnPassou Then
                    If rsUsina("intgec") <> intInstante Then
                        intInstante = rsUsina("intgec")
                        objTextArea.Value = objTextArea.Value & Chr(13)
                    Else
                        objTextArea.Value = objTextArea.Value & Chr(9)
                    End If
                End If
                If Not IsDBNull(rsUsina.Item("valgectran")) Then
                    objTextArea.Value = objTextArea.Value & rsUsina.Item("valgectran")
                Else
    ...
    Private Sub PreencheTable()
        ...
        Try
            ...
            objRow.Controls.Add(objCel)
            tblGEC.Width = objTamanho.Pixel(132)
            tblGEC.Controls.Add(objRow)
            ...
            Do While rsUsina.Read
                If strCodUsina <> rsUsina.Item("codusina") Then
                    ...
                    objCel.Font.Bold = True
                    tblGEC.Rows(0).Width = objTamanho.Pixel(tblGEC.Rows(0).Width.Value + 64)
                    tblGEC.Width = objTamanho.Pixel(tblGEC.Width.Value + 64)
                    ...
                    tblGEC.Rows(0).Controls.Add(objCel)
                ...
                objCel.Width = objTamanho.Pixel(64)
                If Not IsDBNull(rsUsina.Item("valgectran")) Then
                    objCel.Text = rsUsina.Item("valgectran")
                    dblMedia = dblMedia + rsUsina.Item("valgectran")
                Else
                    objCel.Text = 0
                End If
                tblGEC.Rows(intLin).Width = objTamanho.Pixel(tblGEC.Rows(intLin).Width.Value + 63)
                tblGEC.Rows(intLin).Controls.Add(objCel)
                intLin = intLin + 1
                If intLin = 49 Then
                    objCel = New TableCell
                    objCel.Wrap = False
                    ...
                    tblGEC.Rows(intLin).Width = objTamanho.Pixel(tblGEC.Rows(intLin).Width.Value + 64)
                    tblGEC.Rows(intLin).Controls.Add(objCel)
                    tblGEC.Rows(intLin).Cells(1).Text = Trim(Str(Val(tblGEC.Rows(intLin).Cells(1).Text) + dblMedia)) '(dblMedia / 2)))

                    objCel = New TableCell
                    objCel.Wrap = False
                    objCel.Width = objTamanho.Pixel(64)
                    objCel.Text = Trim(Str(Int(dblMedia / 48)))
                    tblGEC.Rows(intLin + 1).Width = objTamanho.Pixel(tblGEC.Rows(intLin + 1).Width.Value + 64)
                    tblGEC.Rows(intLin + 1).Controls.Add(objCel)
                    tblGEC.Rows(intLin + 1).Cells(1).Text = Trim(Str(Val(tblGEC.Rows(intLin + 1).Cells(1).Text) + Int(dblMedia / 48)))
            ...
            For intI = 1 To 50
                dblMedia = 0
                For intJ = 2 To tblGEC.Rows(0).Cells.Count - 1
                    dblMedia = dblMedia + Val(tblGEC.Rows(intI).Cells(intJ).Text)
                Next
                tblGEC.Rows(intI).Cells(1).Text = dblMedia '/ 2
            Next
            rsUsina.Close()
            rsUsina = Nothing
            'Cmd.Connection.Close()
    ...
    Private Sub btnSalvar_Click(ByVal sender As System.Object, ByVal e As System.Web.UI.ImageClickEventArgs) Handles btnSalvar.Click
        ...
        If EstaAutorizado Then
            ...
            Try
                ...
                'Grava evento registrando o recebimento de GEC (DSP)
