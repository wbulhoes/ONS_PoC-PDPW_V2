...
#End Region
    ...
    Private Sub cboEmpresa_SelectedIndexChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cboEmpresa.SelectedIndexChanged
        ...
        Try
            ...
            If Not ValidaLimiteEntradaDados(cboEmpresa.SelectedItem.Value, cboData.SelectedItem.Value, lRetorno, "SOM") Then
    ...
    Private Sub cboUsina_SelectedIndexChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cboUsina.SelectedIndexChanged
        ...
        Try
            If cboUsina.SelectedItem.Text = "Todas as Usinas" Then
                Cmd.CommandText = "SELECT d.valsomtran, d.intsom, d.codusina, u.ordem " &
                                  "FROM tb_som d, usina u " &
                                  "WHERE u.codempre = '" & cboEmpresa.SelectedItem.Value & "' " &
                                  "AND u.codusina = d.codusina " &
                                  "AND d.datpdp = '" & Format(Session("datEscolhida"), "yyyyMMdd") & "' " &
                                  "AND u.flg_recebepdpage = 'S' " &
                                  "ORDER BY d.intsom, u.ordem, d.codusina"
            Else
                Cmd.CommandText = "SELECT d.valsomtran, d.intsom " &
                                  "FROM tb_som d, usina u " &
                                  "WHERE u.codempre = '" & cboEmpresa.SelectedItem.Value & "' " &
                                  "AND u.codusina = d.codusina " &
            ...
            Do While rsUsina.Read
                If blnPassou Then
                    If rsUsina("intsom") <> intInstante Then
                        intInstante = rsUsina("intsom")
                        objTextArea.Value = objTextArea.Value & Chr(13)
                    Else
                        objTextArea.Value = objTextArea.Value & Chr(9)
                    End If
                End If
                If Not IsDBNull(rsUsina.Item("valsomtran")) Then
                    objTextArea.Value = objTextArea.Value & rsUsina.Item("valsomtran")
                Else
    ...
    Private Sub PreencheTable()
        ...
        Dim strCodUsina As String
        Try
            Cmd.CommandText = "SELECT d.codusina, d.intsom, d.valsomtran, u.ordem " &
                              "FROM tb_som d, usina u " &
                              "WHERE u.codempre = '" & Session("strCodEmpre") & "' " &
                              "AND u.codusina = d.codusina " &
                              "AND d.datpdp = '" & Format(Session("datEscolhida"), "yyyyMMdd") & "' " &
                              "AND u.flg_recebepdpage = 'S' " &
                              "ORDER BY u.ordem, d.codusina, d.intsom "
            Conn.Open("pdp")
            Dim rsUsina As OnsClasses.OnsData.OnsDataReader = Cmd.ExecuteReader
            tblSOM.Rows.Clear()
            ...
            objRow.Controls.Add(objCel)
            tblSOM.Width = objTamanho.Pixel(132)
            tblSOM.Controls.Add(objRow)
            ...
            Do While rsUsina.Read
                If strCodUsina <> rsUsina.Item("codusina") Then
                    ...
                    objCel.Font.Bold = True
                    tblSOM.Rows(0).Width = objTamanho.Pixel(tblSOM.Rows(0).Width.Value + 64)
                    tblSOM.Width = objTamanho.Pixel(tblSOM.Width.Value + 64)
                    ...
                    tblSOM.Rows(0).Controls.Add(objCel)
                ...
                objCel.Width = objTamanho.Pixel(64)
                If Not IsDBNull(rsUsina.Item("valsomtran")) Then
                    objCel.Text = rsUsina.Item("valsomtran")
                    dblMedia = dblMedia + rsUsina("valsomtran")
                Else
                    objCel.Text = 0
                End If
                tblSOM.Rows(intLin).Width = objTamanho.Pixel(tblSOM.Rows(intLin).Width.Value + 63)
                tblSOM.Rows(intLin).Controls.Add(objCel)
                intLin = intLin + 1
                If intLin = 49 Then
                    objCel = New TableCell
                    objCel.Wrap = False
                    ...
                    tblSOM.Rows(intLin).Width = objTamanho.Pixel(tblSOM.Rows(intLin).Width.Value + 64)
                    tblSOM.Rows(intLin).Controls.Add(objCel)
                    tblSOM.Rows(intLin).Cells(1).Text = Trim(Str(Val(tblSOM.Rows(intLin).Cells(1).Text) + dblMedia)) '(dblMedia / 2)))

                    objCel = New TableCell
                    objCel.Wrap = False
                    objCel.Width = objTamanho.Pixel(64)
                    objCel.Text = Trim(Str(Int(dblMedia / 48)))
                    tblSOM.Rows(intLin + 1).Width = objTamanho.Pixel(tblSOM.Rows(intLin + 1).Width.Value + 64)
                    tblSOM.Rows(intLin + 1).Controls.Add(objCel)
                    tblSOM.Rows(intLin + 1).Cells(1).Text = Trim(Str(Val(tblSOM.Rows(intLin + 1).Cells(1).Text) + Int(dblMedia / 48)))
            ...
            For intI = 1 To 50
                dblMedia = 0
                For intJ = 2 To tblSOM.Rows(0).Cells.Count - 1
                    dblMedia = dblMedia + Val(tblSOM.Rows(intI).Cells(intJ).Text)
                Next
                tblSOM.Rows(intI).Cells(1).Text = dblMedia '/ 2
            Next
            rsUsina.Close()
            rsUsina = Nothing
            'Cmd.Connection.Close()
    ...
    Private Sub btnSalvar_Click(ByVal sender As System.Object, ByVal e As System.Web.UI.ImageClickEventArgs) Handles btnSalvar.Click
        ...
        'objPermissao = New OnsClasses.OnsInterfaceUnica.OnsPermissao("UPD", "PDPColSom", UsuarID)
        ...
        If EstaAutorizado Then
            ...
            Try
                ...
                If strValor = "" Then
                    ...
                    If cboUsina.SelectedItem.Text <> "Todas as Usinas" Then
                        For intI = 1 To 48
                            ...
                            Cmd.CommandText = "UPDATE tb_som " & _
                                              "SET valsomtran = 0 " & _
                ...
                'Grava evento registrando o recebimento de Geração Substituta (SOM)
                GravaEventoPDP("55", Format(Session("datEscolhida"), "yyyyMMdd"), cboEmpresa.SelectedItem.Text, datAtual, "PDPColSom", UsuarID)
