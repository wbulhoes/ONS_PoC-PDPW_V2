Partial Class frmColImportacao
...
#End Region
    ...
    Private Sub cboEmpresa_SelectedIndexChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cboEmpresa.SelectedIndexChanged
        ...
        Try
            ...
            If Not ValidaLimiteEntradaDados(cboEmpresa.SelectedItem.Value, cboData.SelectedItem.Value, lRetorno, "IMP") Then
    ...
    Private Sub cboUsina_SelectedIndexChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cboUsina.SelectedIndexChanged
        ...
        Try
            If cboUsina.SelectedItem.Text = "Todas as Usinas" Then
                Cmd.CommandText = "Select d.valimportatran, " &
                                "       d.intimporta, " &
                                "       d.codusina, " &
                                "       u.ordem " &
                                "From importa d, " &
            ...
            Do While rsUsina.Read
                If blnPassou Then
                    If rsUsina("intimporta") <> intInstante Then
                        intInstante = rsUsina("intimporta")
                        objTextArea.Value = objTextArea.Value & Chr(13)
                    Else
                        objTextArea.Value = objTextArea.Value & Chr(9)
                    End If
                End If
                If Not IsDBNull(rsUsina.Item("valimportatran")) Then
                    objTextArea.Value = objTextArea.Value & rsUsina.Item("valimportatran")
                Else
    ...
    Private Sub PreencheTable()
        ...
        Try
            ...
            objRow.Controls.Add(objCel)
            tblImportacao.Width = objTamanho.Pixel(132)
            tblImportacao.Controls.Add(objRow)
            ...
            Do While rsUsina.Read
                If strCodUsina <> rsUsina.Item("codusina") Then
                    ...
                    objCel.Font.Bold = True
                    tblImportacao.Rows(0).Width = objTamanho.Pixel(tblImportacao.Rows(0).Width.Value + 64)
                    tblImportacao.Width = objTamanho.Pixel(tblImportacao.Width.Value + 64)
                    ...
                    tblImportacao.Rows(0).Controls.Add(objCel)
                ...
                objCel.Width = objTamanho.Pixel(64)
                If Not IsDBNull(rsUsina.Item("valimportatran")) Then
                    objCel.Text = rsUsina.Item("valimportatran")
                    dblMedia = dblMedia + rsUsina.Item("valimportatran")
                Else
                    objCel.Text = 0
                End If
                tblImportacao.Rows(intLin).Width = objTamanho.Pixel(tblImportacao.Rows(intLin).Width.Value + 63)
                tblImportacao.Rows(intLin).Controls.Add(objCel)
                intLin = intLin + 1
                If intLin = 49 Then
                    objCel = New TableCell
                    objCel.Wrap = False
                    ...
                    tblImportacao.Rows(intLin).Width = objTamanho.Pixel(tblImportacao.Rows(intLin).Width.Value + 64)
                    tblImportacao.Rows(intLin).Controls.Add(objCel)
                    tblImportacao.Rows(intLin).Cells(1).Text = Trim(Str(Val(tblImportacao.Rows(intLin).Cells(1).Text) + dblMedia)) '(dblMedia / 2)))

                    objCel = New TableCell
                    objCel.Wrap = False
                    objCel.Width = objTamanho.Pixel(64)
                    objCel.Text = Trim(Str(Int(dblMedia / 48)))
                    tblImportacao.Rows(intLin + 1).Width = objTamanho.Pixel(tblImportacao.Rows(intLin + 1).Width.Value + 64)
                    tblImportacao.Rows(intLin + 1).Controls.Add(objCel)
                    tblImportacao.Rows(intLin + 1).Cells(1).Text = Trim(Str(Val(tblImportacao.Rows(intLin + 1).Cells(1).Text) + Int(dblMedia / 48)))
            ...
            For intI = 1 To 50
                dblMedia = 0
                For intJ = 2 To tblImportacao.Rows(0).Cells.Count - 1
                    dblMedia = dblMedia + Val(tblImportacao.Rows(intI).Cells(intJ).Text)
                Next
                tblImportacao.Rows(intI).Cells(1).Text = dblMedia '/ 2
            Next
    ...
    Private Sub btnSalvar_Click(ByVal sender As System.Object, ByVal e As System.Web.UI.ImageClickEventArgs) Handles btnSalvar.Click
        ...
        'objPermissao = New OnsClasses.OnsInterfaceUnica.OnsPermissao("UPD", "PDPColImporta", UsuarID)
        ...
        If EstaAutorizado Then
            ...
            Try
                ...
                If strValor = "" Then
                    ...
                    If cboUsina.SelectedItem.Text <> "Todas as Usinas" Then
                        For intI = 1 To 48
                            ...
                            Cmd.CommandText = "Update importa " & _
                                                "Set valimportatran = 0 " & _
                ...
                'Grava evento registrando o recebimento de importaçãp
                GravaEventoPDP("32", Format(Session("datEscolhida"), "yyyyMMdd"), cboEmpresa.SelectedItem.Text, datAtual, "PDPColImporta", UsuarID)
