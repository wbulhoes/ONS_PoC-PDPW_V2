...
#End Region
    ...
    Private Sub cboEmpresa_SelectedIndexChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cboEmpresa.SelectedIndexChanged
        ...
        Try
            ...
            If Not ValidaLimiteEntradaDados(cboEmpresa.SelectedItem.Value, cboData.SelectedItem.Value, lRetorno, "R12") Then
    ...
    Private Sub cboUsina_SelectedIndexChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cboUsina.SelectedIndexChanged
        ...
        Try
            If cboUsina.SelectedItem.Text = "Todas as Usinas" Then
                Cmd.CommandText = "Select d.valir3tran, " &
                                "       d.intir3, " &
                                "       d.codusina, " &
                                "       u.ordem " &
                                "From tb_IR3 d, " &
            ...
            Do While rsUsina.Read
                If blnPassou Then
                    If rsUsina("intir3") <> intInstante Then
                        intInstante = rsUsina("intir3")
                        objTextArea.Value = objTextArea.Value & Chr(13)
                    Else
                        objTextArea.Value = objTextArea.Value & Chr(9)
                    End If
                End If
                If Not IsDBNull(rsUsina.Item("valir3tran")) Then
                    objTextArea.Value = objTextArea.Value & rsUsina.Item("valir3tran")
                Else
    ...
    Private Sub PreencheTable()
        ...
        Try
            ...
            objRow.Controls.Add(objCel)
            tblIR3.Width = objTamanho.Pixel(132)
            tblIR3.Controls.Add(objRow)
            ...
            Do While rsUsina.Read
                If strCodUsina <> rsUsina.Item("codusina") Then
                    ...
                    objCel.Font.Bold = True
                    tblIR3.Rows(0).Width = objTamanho.Pixel(tblIR3.Rows(0).Width.Value + 64)
                    tblIR3.Width = objTamanho.Pixel(tblIR3.Width.Value + 64)
                    ...
                    tblIR3.Rows(0).Controls.Add(objCel)
                ...
                objCel.Width = objTamanho.Pixel(64)
                If Not IsDBNull(rsUsina.Item("valir3tran")) Then
                    objCel.Text = rsUsina.Item("valir3tran")
                    dblMedia = dblMedia + rsUsina.Item("valir3tran")
                Else
                    objCel.Text = 0
                End If
                tblIR3.Rows(intLin).Width = objTamanho.Pixel(tblIR3.Rows(intLin).Width.Value + 63)
                tblIR3.Rows(intLin).Controls.Add(objCel)
                intLin = intLin + 1
                If intLin = 49 Then
                    objCel = New TableCell
                    objCel.Wrap = False
                    ...
                    tblIR3.Rows(intLin).Width = objTamanho.Pixel(tblIR3.Rows(intLin).Width.Value + 64)
                    tblIR3.Rows(intLin).Controls.Add(objCel)
                    tblIR3.Rows(intLin).Cells(1).Text = Trim(Str(Val(tblIR3.Rows(intLin).Cells(1).Text) + dblMedia)) '(dblMedia / 2)))

                    objCel = New TableCell
                    objCel.Wrap = False
                    objCel.Width = objTamanho.Pixel(64)
                    objCel.Text = Trim(Str(Int(dblMedia / 48)))
                    tblIR3.Rows(intLin + 1).Width = objTamanho.Pixel(tblIR3.Rows(intLin + 1).Width.Value + 64)
                    tblIR3.Rows(intLin + 1).Controls.Add(objCel)
                    tblIR3.Rows(intLin + 1).Cells(1).Text = Trim(Str(Val(tblIR3.Rows(intLin + 1).Cells(1).Text) + Int(dblMedia / 48)))
            ...
            For intI = 1 To 50
                dblMedia = 0
                For intJ = 2 To tblIR3.Rows(0).Cells.Count - 1
                    dblMedia = dblMedia + Val(tblIR3.Rows(intI).Cells(intJ).Text)
                Next
                tblIR3.Rows(intI).Cells(1).Text = dblMedia '/ 2
            Next
            rsUsina.Close()
            rsUsina = Nothing
            'Cmd.Connection.Close()
    ...
    Private Sub btnSalvar_Click(ByVal sender As System.Object, ByVal e As System.Web.UI.ImageClickEventArgs) Handles btnSalvar.Click
        ...
        If EstaAutorizado Then
            ...
            Try
                ...
                'Grava evento registrando o recebimento de IR3 (DSP)
                GravaEventoPDP("62", Format(Session("datEscolhida"), "yyyyMMdd"), cboEmpresa.SelectedItem.Text, datAtual, "PDPColDsp", UsuarID)
