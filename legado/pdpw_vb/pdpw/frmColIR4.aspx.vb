...
#End Region
    ...
    Private Sub cboEmpresa_SelectedIndexChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cboEmpresa.SelectedIndexChanged
        ...
        Try
            ...
            If Not ValidaLimiteEntradaDados(cboEmpresa.SelectedItem.Value, cboData.SelectedItem.Value, lRetorno, "CAN") Then
    ...
    Private Sub cboUsina_SelectedIndexChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cboUsina.SelectedIndexChanged
        ...
        Try
            If cboUsina.SelectedItem.Text = "Todas as Usinas" Then
                Cmd.CommandText = "Select d.valir4tran, " &
                                "       d.intir4, " &
                                "       d.codusina, " &
                                "       u.ordem " &
                                "From tb_IR4 d, " &
            ...
            Do While rsUsina.Read
                If blnPassou Then
                    If rsUsina("intir4") <> intInstante Then
                        intInstante = rsUsina("intir4")
                        objTextArea.Value = objTextArea.Value & Chr(13)
                    Else
                        objTextArea.Value = objTextArea.Value & Chr(9)
                    End If
                End If
                If Not IsDBNull(rsUsina.Item("valir4tran")) Then
                    objTextArea.Value = objTextArea.Value & rsUsina.Item("valir4tran")
                Else
    ...
    Private Sub PreencheTable()
        ...
        Try
            ...
            objRow.Controls.Add(objCel)
            tblIR4.Width = objTamanho.Pixel(132)
            tblIR4.Controls.Add(objRow)
            ...
            Do While rsUsina.Read
                If strCodUsina <> rsUsina.Item("codusina") Then
                    ...
                    objCel.Font.Bold = True
                    tblIR4.Rows(0).Width = objTamanho.Pixel(tblIR4.Rows(0).Width.Value + 64)
                    tblIR4.Width = objTamanho.Pixel(tblIR4.Width.Value + 64)
                    ...
                    tblIR4.Rows(0).Controls.Add(objCel)
                ...
                objCel.Width = objTamanho.Pixel(64)
                If Not IsDBNull(rsUsina.Item("valir4tran")) Then
                    objCel.Text = rsUsina.Item("valir4tran")
                    dblMedia = dblMedia + rsUsina.Item("valir4tran")
                Else
                    objCel.Text = 0
                End If
                tblIR4.Rows(intLin).Width = objTamanho.Pixel(tblIR4.Rows(intLin).Width.Value + 63)
                tblIR4.Rows(intLin).Controls.Add(objCel)
                intLin = intLin + 1
                If intLin = 49 Then
                    objCel = New TableCell
                    objCel.Wrap = False
                    ...
                    tblIR4.Rows(intLin).Width = objTamanho.Pixel(tblIR4.Rows(intLin).Width.Value + 64)
                    tblIR4.Rows(intLin).Controls.Add(objCel)
                    tblIR4.Rows(intLin).Cells(1).Text = Trim(Str(Val(tblIR4.Rows(intLin).Cells(1).Text) + dblMedia)) '(dblMedia / 2)))

                    objCel = New TableCell
                    objCel.Wrap = False
                    objCel.Width = objTamanho.Pixel(64)
                    objCel.Text = Trim(Str(Int(dblMedia / 48)))
                    tblIR4.Rows(intLin + 1).Width = objTamanho.Pixel(tblIR4.Rows(intLin + 1).Width.Value + 64)
                    tblIR4.Rows(intLin + 1).Controls.Add(objCel)
                    tblIR4.Rows(intLin + 1).Cells(1).Text = Trim(Str(Val(tblIR4.Rows(intLin + 1).Cells(1).Text) + Int(dblMedia / 48)))
            ...
            For intI = 1 To 50
                dblMedia = 0
                For intJ = 2 To tblIR4.Rows(0).Cells.Count - 1
                    dblMedia = dblMedia + Val(tblIR4.Rows(intI).Cells(intJ).Text)
                Next
                tblIR4.Rows(intI).Cells(1).Text = dblMedia '/ 2
            Next
            rsUsina.Close()
            rsUsina = Nothing
            'Cmd.Connection.Close()
    ...
    Private Sub btnSalvar_Click(ByVal sender As System.Object, ByVal e As System.Web.UI.ImageClickEventArgs) Handles btnSalvar.Click
        ...
        If EstaAutorizado Then
            ...
            Try
                ...
                'Grava evento registrando o recebimento de IR4 (DSP)
                GravaEventoPDP("63", Format(Session("datEscolhida"), "yyyyMMdd"), cboEmpresa.SelectedItem.Text, datAtual, "PDPColDsp", UsuarID)
