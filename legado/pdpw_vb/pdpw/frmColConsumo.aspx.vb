...
#End Region
    ...
    Private Sub cboEmpresa_SelectedIndexChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cboEmpresa.SelectedIndexChanged
        ...
        Try
            ...
            If Not ValidaLimiteEntradaDados(cboEmpresa.SelectedItem.Value, cboData.SelectedItem.Value, lRetorno, "PCC") Then
    ...
    Private Sub cboUsina_SelectedIndexChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cboUsina.SelectedIndexChanged
        ...
        Try
            If cboUsina.SelectedItem.Text = "Todas as Usinas" Then
                Cmd.CommandText = "Select d.valpcctran, " &
                                "       d.intpcc, " &
                                "       d.codusina, " &
                                "       u.ordem " &
                                "From perdascic d, " &
            ...
            Do While rsUsina.Read
                If blnPassou Then
                    If rsUsina("intpcc") <> intInstante Then
                        intInstante = rsUsina("intpcc")
                        objTextArea.Value = objTextArea.Value & Chr(13)
                    Else
                        objTextArea.Value = objTextArea.Value & Chr(9)
                    End If
                End If
                If Not IsDBNull(rsUsina.Item("valpcctran")) Then
                    objTextArea.Value = objTextArea.Value & rsUsina.Item("valpcctran")
                Else
    ...
    Private Sub PreencheTable()
        ...
        Try
            ...
            objRow.Controls.Add(objCel)
            tblConsumo.Width = objTamanho.Pixel(132)
            tblConsumo.Controls.Add(objRow)
            ...
            Do While rsUsina.Read
                If strCodUsina <> rsUsina.Item("codusina") Then
                    ...
                    objCel.Font.Bold = True
                    tblConsumo.Rows(0).Width = objTamanho.Pixel(tblConsumo.Rows(0).Width.Value + 64)
                    tblConsumo.Width = objTamanho.Pixel(tblConsumo.Width.Value + 64)
                    ...
                    tblConsumo.Rows(0).Controls.Add(objCel)
                ...
                objCel.Width = objTamanho.Pixel(64)
                If Not IsDBNull(rsUsina.Item("valpcctran")) Then
                    objCel.Text = rsUsina.Item("valpcctran")
                    dblMedia = dblMedia + rsUsina.Item("valpcctran")
                Else
                    objCel.Text = 0
                End If
                tblConsumo.Rows(intLin).Width = objTamanho.Pixel(tblConsumo.Rows(intLin).Width.Value + 63)
                tblConsumo.Rows(intLin).Controls.Add(objCel)
                intLin = intLin + 1
                If intLin = 49 Then
                    objCel = New TableCell
                    objCel.Wrap = False
                    ...
                    tblConsumo.Rows(intLin).Width = objTamanho.Pixel(tblConsumo.Rows(intLin).Width.Value + 64)
                    tblConsumo.Rows(intLin).Controls.Add(objCel)
                    tblConsumo.Rows(intLin).Cells(1).Text = Trim(Str(Val(tblConsumo.Rows(intLin).Cells(1).Text) + dblMedia))  ' (dblMedia / 2)))

                    objCel = New TableCell
                    objCel.Wrap = False
                    objCel.Width = objTamanho.Pixel(64)
                    objCel.Text = Trim(Str(Int(dblMedia / 48)))
                    tblConsumo.Rows(intLin + 1).Width = objTamanho.Pixel(tblConsumo.Rows(intLin + 1).Width.Value + 64)
                    tblConsumo.Rows(intLin + 1).Controls.Add(objCel)
                    tblConsumo.Rows(intLin + 1).Cells(1).Text = Trim(Str(Val(tblConsumo.Rows(intLin + 1).Cells(1).Text) + Int(dblMedia / 48)))
            ...
            For intI = 1 To 50
                dblMedia = 0
                For intJ = 2 To tblConsumo.Rows(0).Cells.Count - 1
                    dblMedia = dblMedia + Val(tblConsumo.Rows(intI).Cells(intJ).Text)
                Next
                tblConsumo.Rows(intI).Cells(1).Text = dblMedia '/ 2
            Next
    ...
    Private Sub btnSalvar_Click(ByVal sender As System.Object, ByVal e As System.Web.UI.ImageClickEventArgs) Handles btnSalvar.Click
        ...
        'objPermissao = New OnsClasses.OnsInterfaceUnica.OnsPermissao("UPD", "PDPColConsumo", UsuarID)
        ...
        If EstaAutorizado Then
            ...
            Try
                ...
                If strValor = "" Then
                    ...
                    If cboUsina.SelectedItem.Text <> "Todas as Usinas" Then
                        For intI = 1 To 48
                            ...
                            Cmd.CommandText = "Update perdascic " & _
                                                "Set valpcctran = 0 " & _
                ...
                'Grava evento registrando o recebimento de Perdas Consumo Interno e Compensação
                GravaEventoPDP("35", Format(Session("datEscolhida"), "yyyyMMdd"), cboEmpresa.SelectedItem.Text, datAtual, "PDPColConsumo", UsuarID)
