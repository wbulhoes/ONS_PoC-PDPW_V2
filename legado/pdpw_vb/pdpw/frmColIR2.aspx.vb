...
#End Region
    ...
    Private Sub cboEmpresa_SelectedIndexChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cboEmpresa.SelectedIndexChanged
        ...
        Try
            ...
            If Not ValidaLimiteEntradaDados(cboEmpresa.SelectedItem.Value, cboData.SelectedItem.Value, lRetorno, "R11") Then
    ...
    Private Sub cboUsina_SelectedIndexChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cboUsina.SelectedIndexChanged
        ...
        Try
            If cboUsina.SelectedItem.Text = "Todas as Usinas" Then
                Cmd.CommandText = "Select d.valir2tran, " &
                                "       d.intir2, " &
                                "       d.codusina, " &
                                "       u.ordem " &
                                "From tb_IR2 d, " &
            ...
            Do While rsUsina.Read
                If blnPassou Then
                    If rsUsina("intir2") <> intInstante Then
                        intInstante = rsUsina("intir2")
                        objTextArea.Value = objTextArea.Value & Chr(13)
                    Else
                        objTextArea.Value = objTextArea.Value & Chr(9)
                    End If
                End If
                If Not IsDBNull(rsUsina.Item("valir2tran")) Then
                    objTextArea.Value = objTextArea.Value & rsUsina.Item("valir2tran")
                Else
    ...
    Private Sub PreencheTable()
        ...
        Try
            ...
            objRow.Controls.Add(objCel)
            tblIR2.Width = objTamanho.Pixel(132)
            tblIR2.Controls.Add(objRow)
            ...
            Do While rsUsina.Read
                If strCodUsina <> rsUsina.Item("codusina") Then
                    ...
                    objCel.Font.Bold = True
                    tblIR2.Rows(0).Width = objTamanho.Pixel(tblIR2.Rows(0).Width.Value + 64)
                    tblIR2.Width = objTamanho.Pixel(tblIR2.Width.Value + 64)
                    ...
                    tblIR2.Rows(0).Controls.Add(objCel)
                ...
                objCel.Width = objTamanho.Pixel(64)
                If Not IsDBNull(rsUsina.Item("valir2tran")) Then
                    objCel.Text = rsUsina.Item("valir2tran")
                    dblMedia = dblMedia + rsUsina.Item("valir2tran")
                Else
                    objCel.Text = 0
                End If
                tblIR2.Rows(intLin).Width = objTamanho.Pixel(tblIR2.Rows(intLin).Width.Value + 63)
                tblIR2.Rows(intLin).Controls.Add(objCel)
                intLin = intLin + 1
                If intLin = 49 Then
                    objCel = New TableCell
                    objCel.Wrap = False
                    ...
                    tblIR2.Rows(intLin).Width = objTamanho.Pixel(tblIR2.Rows(intLin).Width.Value + 64)
                    tblIR2.Rows(intLin).Controls.Add(objCel)
                    tblIR2.Rows(intLin).Cells(1).Text = Trim(Str(Val(tblIR2.Rows(intLin).Cells(1).Text) + dblMedia)) '(dblMedia / 2)))

                    objCel = New TableCell
                    objCel.Wrap = False
                    objCel.Width = objTamanho.Pixel(64)
                    objCel.Text = Trim(Str(Int(dblMedia / 48)))
                    tblIR2.Rows(intLin + 1).Width = objTamanho.Pixel(tblIR2.Rows(intLin + 1).Width.Value + 64)
                    tblIR2.Rows(intLin + 1).Controls.Add(objCel)
                    tblIR2.Rows(intLin + 1).Cells(1).Text = Trim(Str(Val(tblIR2.Rows(intLin + 1).Cells(1).Text) + Int(dblMedia / 48)))
            ...
            For intI = 1 To 50
                dblMedia = 0
                For intJ = 2 To tblIR2.Rows(0).Cells.Count - 1
                    dblMedia = dblMedia + Val(tblIR2.Rows(intI).Cells(intJ).Text)
                Next
                tblIR2.Rows(intI).Cells(1).Text = dblMedia '/ 2
            Next
            rsUsina.Close()
            rsUsina = Nothing
            'Cmd.Connection.Close()
    ...
    Private Sub btnSalvar_Click(ByVal sender As System.Object, ByVal e As System.Web.UI.ImageClickEventArgs) Handles btnSalvar.Click
        ...
        If EstaAutorizado Then
            ...
            Try
                ...
                'Grava evento registrando o recebimento de IR2 (DSP)
                GravaEventoPDP("61", Format(Session("datEscolhida"), "yyyyMMdd"), cboEmpresa.SelectedItem.Text, datAtual, "PDPColDsp", UsuarID)
