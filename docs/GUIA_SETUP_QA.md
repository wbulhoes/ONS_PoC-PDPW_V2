...
**Projeto:** POC de Migra��o PDPw (VB.NET ? .NET 8 + React)  
**Vers�o:** 2.0  
**Data:** 20/12/2024  
**Respons�vel Backend:** Willian Bulh�es  
**Para:** Equipe de QA
...
- 58 registros fict�cios (dados de teste)
...
### **Reposit�rio Principal (Branch: feature/backend)**
```
https://github.com/wbulhoes/ONS_PoC-PDPW_V2/tree/feature/backend
```

**Cont�m:**
- C�digo-fonte completo do backend (.NET 8)
- Docker Compose
- Migrations do EF Core
- Scripts SQL
- Documenta��o

### **Reposit�rio Fork (Branch: feature/apis-implementadas)**
```
https://github.com/wbulhoes/POCMigracaoPDPw/tree/feature/apis-implementadas
```

**Cont�m:**
- Mesmo c�digo sincronizado
...
| Software | Vers�o M�nima | Link | Obrigat�rio |
|----------|---------------|------|-------------|
...
### **Extens�es VS Code Recomendadas:**
- C# Dev Kit
- Docker
- PowerShell
- REST Client
...
# Deve retornar vers�es similares a:
...
# Aguardar ~2 minutos para inicializa��o completa
```
...
? Se ver a interface do Swagger, o ambiente est� OK!
...
?   ?   ??? generate-test-data.sql     # Dados fict�cios (58 registros)
?   ?   ??? ...
?   ??? test/                          # Scripts de teste
?       ??? Test-AllApis.ps1           # Testes b�sicos (37 testes)
...
### **OP��O 1: Testes B�sicos (37 testes)**
...
### **OP��O 2: Testes Completos (55 testes) ? RECOMENDADO**
...
### **OP��O 3: Testes Manuais via Swagger**
...
? Coverage:
  ...
  � Valida��es: Dados inv�lidos, duplica��es, ranges
...
| Tabela | Registros Reais | Registros Fict�cios | Total |
|--------|-----------------|---------------------|-------|
| Empresas | 25 | 5 | 30 |
| Usinas | 40 | 10 | 50 |
| TiposUsina | 8 | 0 | 8 |
...
### **Gerar Dados Fict�cios (Opcional)**

```powershell
# Executar script de gera��o de dados de teste
...
**Causa:** Foreign Key inv�lida (ex: `semanaPMOId: 0`)

**Solu��o:** Use IDs v�lidos conforme tabela acima

**Exemplo correto:**
```json
{
  "nomeArquivo": "teste.dat",
  "semanaPMOId": 3,  // ? ID v�lido
  "dataImportacao": "2025-12-21T10:00:00.000Z"
}
```

### **Problema 3: Swagger n�o carrega**
...
### **1. Relat�rio de Valida��o Completa**
...
### **2. Relat�rio de Testes Automatizados**
?? `docs/relatorio-testes-automatizados.md`
...
### **3. Relat�rio de Testes Completos** ?
?? `docs/relatorio-testes-completos.md`
...
- Valida��es e performance

### **4. Guia de Corre��o de Erros**
?? `docs/CORRECAO_ERRO_TESTE_API.md`

**Cont�m:**
- Erros comuns e solu��es
- IDs v�lidos para cada API
...
### **Cen�rio 1: CRUD Completo de Cargas**
...
### **Cen�rio 2: Processamento de Arquivo DADGER**
...
### **Cen�rio 3: Valida��es de Dados Inv�lidos**

```powershell
# 1. Subsistema inv�lido
...
- **Reposit�rio Principal:** https://github.com/wbulhoes/ONS_PoC-PDPW_V2/tree/feature/backend
- **Reposit�rio Fork:** https://github.com/wbulhoes/POCMigracaoPDPw/tree/feature/apis-implementadas

### **Arquivos Importantes**

| Arquivo | Descri��o |
|---------|-----------|
| `docker-compose.full.yml` | Configura��o Docker completa |
| `appsettings.json` | Configura��o da aplica��o |
| `scripts/test/Test-AllApis-Complete.ps1` | Suite completa de testes |
| `docs/RELATORIO_VALIDACAO_COMPLETA.md` | Relat�rio de valida��o |
...
? **Banco:** 259 registros (201 reais + 58 fict�cios)  
? **Testes:** 55 testes automatizados (89% sucesso)  
? **Docs:** Relat�rios completos e atualizados  
? **Infra:** Docker + SQL Server configurados  

**Taxa de Sucesso Esperada:** ~89%  
**Performance:** ~10ms por requisi��o  
...
1. Consulte os relat�rios em `docs/`
2. Verifique o Troubleshooting acima
3. Analise os logs: `docker logs pdpw-backend`
4. Entre em contato com Willian Bulh�es

---

**�ltima Atualiza��o:** 20/12/2024 22:30  
